...
def main():
    ...
    for rank, (_, row) in enumerate(selected.head(40).iterrows(), 1):
        ...
        p(f'  │ {rank:>4} {row["ts_code"][:6]:8} {str(row.get("name",""))[:6]:8} {str(row.get("industry",""))[:4]:6} ' f'{row["score"]:>6.4f} {row["pct_chg"]:>+5.1f}% {f.get("ret_5d",0)*100:>+6.1f}% ' f'{f.get("vol_ratio",0):>5.2f} {lim:>3}')
    ...
    for _, row in near_miss.head(20).iterrows():
        ...
        p(f'  │ {row["ts_code"][:6]:8} {str(row.get("name",""))[:6]:8} {row["score"]:>6.4f} ' f'{row["pct_chg"]:>+5.1f}% {f.get("ret_5d",0)*100:>+6.1f}% {gap:>+5.4f}')
    ...
    if limit_in_pool > 0:
        ...
        for _, row in limit_pool.head(15).iterrows():
            ...
            p(f'  {row["ts_code"][:6]:8} {str(row.get("name",""))[:6]:8} {row["score"]:>6.4f} ' f'{row["pct_chg"]:>+5.1f}% {f.get("ret_5d",0)*100:>+6.1f}% {f.get("vol_ratio",0):>5.2f} {dec:>10}')
    ...
    for _, row in rev_high.head(15).iterrows():
        f = stock_factors.get(row["ts_code"], {})
        dec = "SELECTED" if row["score"] >= SELECT_THRESHOLD else ("NEAR_MISS" if row["score"] >= NEAR_MISS_THRESHOLD else "watch")
        p(f'  {row["ts_code"][:6]:8} {str(row.get("name",""))[:6]:8} {row["score"]:>6.4f} ' f'{row["pct_chg"]:>+5.1f}% {f.get("ret_5d",0)*100:>+6.1f}% {row["str_val"]:>5.3f} {dec:>10}')
    ...
    for rank, (_, row) in enumerate(selected.head(10).iterrows(), 1):
        ...
        p(f'  技术特征: 5日={f.get("ret_5d",0)*100:+.1f}% 10日={f.get("ret_10d",0)*100:+.1f}%  ' f'量比={f.get("vol_ratio",0):.2f}  阳线={"是" if f.get("is_bull") else "否"}  ' f'波动率={f.get("volatility",0):.4f}')
    ...
    for hist_date in lookback_dates[:-1]:  # 排除当天
        ...
        try:
            ...
            if hd is None or hn is None or hd.empty or hn.empty:
                continue
        except Exception:
            continue
    ...
    for rank, (_, row) in enumerate(high_conf.head(15).iterrows(), 1):
        f = stock_factors.get(row["ts_code"], {})
        p(f'  {rank:>4} {row["ts_code"][:6]:8} {str(row.get("name",""))[:6]:8} {row["score"]:>6.4f} ' f'{row["pct_chg"]:>+5.1f}% {f.get("close_strength",0):>5.2f} {f.get("catalyst_freshness",0):>5.2f} ' f'{f.get("volume_expansion_quality",0):>5.2f}')

    p("\n  交易建议:")
    p("  1. 优先从高确信标的中选择, score≥0.60的标的更有把握")
    p("  2. 反转信号标的(5日跌>8%)适合博反弹, 但需控制仓位")
    p("  3. 涨停股次日高开概率大, 但需注意冲高回落风险")
    p(f'  4. 市场状态: {trade_date}均涨{results["pct_chg"].mean():+.2f}%, ' f'{"偏强→可适当进攻" if results["pct_chg"].mean() > 0.5 else "中性→精选为主" if results["pct_chg"].mean() > -0.5 else "偏弱→控制仓位"}')
        ...
        "reversal_candidates": [
            {
                ...
                "name": str(r.get("name", "")),
                "score": round(float(r["score"]), 4),
                "reversal": round(float(r.get("str_val", 0)), 4),
