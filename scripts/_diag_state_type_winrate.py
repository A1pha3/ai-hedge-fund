...
def main() -> int:
    ...
    q1, q2_rows, q3, verdict = run_state_type_diagnosis(reports_dir=args.reports_dir, lookback_days=args.lookback_days)
    ...
    for r in q1.rows:
        ...
        print(f"  {r.state_type:<8} winrate={wr:<6} median={med:<8} " f"(mature n={r.mature_t30_count}, all n={r.sample_count})")
    ...
    print(f"\n[问3] 留一时段样本外验证: robust={q3.robust} " f"maintained_rate={q3.rediscovered_winner_rate:.0%} " f"({q3.heldout_periods} periods)")
