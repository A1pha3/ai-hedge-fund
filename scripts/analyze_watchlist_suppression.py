...
def _build_variant_summary(variant_name: str) -> dict:
    return {
...
def _process_variant_trade_date(summary: dict, *, trade_date: str, baseline_row: dict, variant_row: dict) -> None:
    base_filters = _filters(baseline_row)
    var_filters = _filters(variant_row)
    ...
    watchlist_threshold = float(((variant_row.get("current_plan") or {}).get("counts") or {}).get("watchlist_score_threshold", 0.25))
    for ticker in extra_tickers:
        _record_variant_ticker(
            summary,
            trade_date=trade_date,
            ticker=ticker,
            watchlist_map=watchlist_map,
            buy_orders_map=buy_orders_map,
            watchlist_threshold=watchlist_threshold,
        )


def _resolve_variant_bucket(ticker: str, *, watchlist_map: dict[str, dict], buy_orders_map: dict[str, dict], summary: dict) -> tuple[str, dict]:
    if ticker in watchlist_map:
        summary["blocked_at_watchlist"] += 1
        return "watchlist", watchlist_map[ticker]
    if ticker in buy_orders_map:
        summary["blocked_at_buy_orders"] += 1
        return "buy_orders", buy_orders_map[ticker]
    summary["missing_from_filters"] += 1
    return "missing_from_filters", {}


def _record_variant_ticker(
    summary: dict,
    *,
    trade_date: str,
    ticker: str,
    watchlist_map: dict[str, dict],
    buy_orders_map: dict[str, dict],
    watchlist_threshold: float,
) -> None:
    summary["extra_layer_b_total"] += 1
    summary["ticker_counts"][ticker] += 1
    bucket, payload = _resolve_variant_bucket(ticker, watchlist_map=watchlist_map, buy_orders_map=buy_orders_map, summary=summary)
    ...
    _record_variant_scores(summary, payload, trade_date=trade_date, ticker=ticker, bucket=bucket, reasons=reasons, watchlist_threshold=watchlist_threshold)


def _record_variant_scores(
    summary: dict,
    payload: dict,
    *,
    trade_date: str,
...
def analyze_variant(baseline_rows: dict[str, dict], variant_rows: dict[str, dict], variant_name: str) -> dict:
    summary = _build_variant_summary(variant_name)
    for trade_date in sorted(set(baseline_rows) & set(variant_rows)):
        _process_variant_trade_date(
            summary,
            trade_date=trade_date,
            baseline_row=baseline_rows[trade_date],
            variant_row=variant_rows[trade_date],
        )
    return summary
