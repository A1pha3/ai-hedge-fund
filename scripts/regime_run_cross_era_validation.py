...
- 夜刷链接线 (R183 Op1, 取代本工具出生时的「一次性 owner 触发工件,
  不进夜刷链」排除决定, commit 7c7f9bdb): 该排除理由对当前侧为假 —
  当前表每夜重建, 本工具的当前侧 CI/点罚分/verdict 随之变化, 报告冻结
  在手动运行日正是判定输入当期化原则的反面; 早期侧仍冻结 (数字不随新
  数据变化仅对早期侧成立)。夜刷链内 fail-open 家族纪律 (R133 先例):
  失败只进 status["diagnostics"], 不阻断夜刷链生产步骤。
