...
def _collect_handoff_focus_tickers(
    failure_dossier: dict[str, Any],
    watchlist_dossier: dict[str, Any],
    recall_dossier: dict[str, Any],
) -> list[str]:
    focus_tickers: list[str] = []
    for source in (
        list(failure_dossier.get("top_upstream_absence_tickers") or []),
        list(watchlist_dossier.get("focus_tickers") or []),
        list(recall_dossier.get("focus_tickers") or []),
    ):
...
def _build_upstream_handoff_board_row(
    *,
    ticker: str,
    ...
    latest_followup_row: dict[str, Any],
    latest_active_report_dir: str,
) -> dict[str, Any]:
    ...
    if latest_followup_row and is_active_followup:
        ...
        next_step = _build_validated_followup_next_step(
            ticker,
            latest_followup_row,
            priority_handoff=truncation_profile.get("priority_handoff"),
        )
...
def _summarize_upstream_handoff_board(board_rows: list[dict[str, Any]], focus_tickers: list[str]) -> tuple[dict[str, Any], str, str, list[str]]:
    stage_summary = {
...
def analyze_btst_candidate_pool_upstream_handoff_board(
    ...
    *,
    ...
    focus_tickers = _collect_handoff_focus_tickers(failure_dossier, watchlist_dossier, recall_dossier)
    ...
    for ticker in focus_tickers:
        ...
        board_rows.append(
            _build_upstream_handoff_board_row(
                ticker=ticker,
                failure_row=failure_row,
                watchlist_row=watchlist_row,
                recall_row=recall_row,
                prototype=prototype,
                latest_followup_row=latest_followup_row,
                latest_active_report_dir=latest_active_report_dir,
            )
        )
    ...
    stage_summary, board_status, recommendation, next_actions = _summarize_upstream_handoff_board(board_rows, focus_tickers)
