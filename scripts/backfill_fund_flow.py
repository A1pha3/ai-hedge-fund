...
def main():
    ...
    parser.add_argument("--fresh-days", type=int, default=_RESUME_FRESH_DAYS, help="resume 近期阈值 (天)")
    ...
    if not tickers:
        logger.error("无候选 ticker — 请先跑 --auto 生成报告, 或指定 --report")
        return 1

    logger.info("候选 %d 只 ticker, 开始 backfill (rate_limit=%.2fs)", len(tickers), args.rate_limit)
