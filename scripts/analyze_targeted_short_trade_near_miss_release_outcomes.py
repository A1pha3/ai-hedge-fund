...
def analyze_targeted_short_trade_near_miss_release_outcomes(
    ...
    for row in list(release_analysis.get("changed_cases") or []):
        ...
        if promoted and next_close_return is not None and float(next_close_return) > 0:
            promotion_verdict = "selected_with_positive_close"
        elif promoted and next_high_return is not None and float(next_high_return) >= next_high_hit_threshold:
            promotion_verdict = "selected_with_intraday_upside"
        elif promoted:
            promotion_verdict = "selected_but_outcome_mixed"
        else:
            promotion_verdict = "not_selected"
    ...
    if target_cases and promoted_target_case_count == len(target_cases) and positive_next_close_count == len(target_cases):
        ...
        recommendation = "当前 near-miss promotion 至少兑现了稳定的 intraday upside，但收盘延续仍需继续观察。"
    elif target_cases:
        recommendation = "当前 near-miss promotion 已发生变化，但真实次日表现没有形成一致支持，建议只保留为对照样本。"
    else:
        recommendation = "当前没有可供评估的 near-miss promotion 样本。"
