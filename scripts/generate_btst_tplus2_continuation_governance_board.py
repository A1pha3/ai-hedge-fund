...
def generate_btst_tplus2_continuation_governance_board(
    ...
    *,
    ...
    eligible_tickers = list(lane_rulepack.get("eligible_tickers") or lane_validation.get("eligible_tickers") or [])
    watchlist_tickers = list(lane_rulepack.get("watchlist_tickers") or [])
    ...
    if len(eligible_tickers) <= 1 and watchlist_tickers:
    ...
    recommendation = (
        ...
        f"Keep the lane at {lane_stage} / {capital_mode} with eligible_tickers={eligible_tickers}. "
        ...
        f"focus promotion gate={promotion_gate.get('gate_verdict')}; do not merge this lane into default BTST."
    )
