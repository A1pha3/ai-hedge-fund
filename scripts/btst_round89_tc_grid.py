...
def run_all_days(pro, all_dates, test_dates):
    ...
    variants = {f"tc{tc_w:.2f}_tc2d{tc2d_w:.2f}": make_variant_profile(tc_w, tc2d_w) for tc_w, tc2d_w in itertools.product(tc_values, tc2d_values)}
    ...
    for di, test_date in enumerate(test_dates):
        ...
        for variant_name, profile in variants.items():
            weights = {factor_key: float(getattr(profile, weight_field, 0.0)) for factor_key, weight_field in bk.PROFILE_WEIGHT_FIELDS.items()}
...
def main():
    ...
    for name, s in summaries.items():
        ...
        rows.append({"tc_w": tc_w, "tc2d_w": tc2d_w, **s})
