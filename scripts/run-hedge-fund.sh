...
show_help() {
    ...
    echo "  --show-default-model    输出当前从 .env 解析出的默认模型后退出"
    echo "  --help                  显示此帮助信息"
...
# 构建命令
COMMAND="uv run python src/main.py --tickers $TICKER"

if [ "$SHOW_DEFAULT_MODEL" = true ]; then
    COMMAND="uv run python src/main.py --show-default-model"
elif [ -n "$MODEL" ]; then
    COMMAND="$COMMAND --model $MODEL"
fi

# 添加日期参数
if [ "$SHOW_DEFAULT_MODEL" != true ] && [ -n "$START_DATE" ]; then
    COMMAND="$COMMAND --start-date $START_DATE"
fi
if [ "$SHOW_DEFAULT_MODEL" != true ] && [ -n "$END_DATE" ]; then
    COMMAND="$COMMAND --end-date $END_DATE"
fi

# 添加分析师参数
if [ "$SHOW_DEFAULT_MODEL" != true ] && [ "$ANALYSTS_ALL" = true ]; then
    COMMAND="$COMMAND --analysts-all"
elif [ "$SHOW_DEFAULT_MODEL" != true ] && [ -n "$ANALYSTS" ]; then
    COMMAND="$COMMAND --analysts $ANALYSTS"
fi

# 添加Ollama参数
if [ "$SHOW_DEFAULT_MODEL" != true ] && [ "$OLAMA" = true ]; then
...
# 添加显示参数
if [ "$SHOW_DEFAULT_MODEL" != true ] && [ "$SHOW_REASONING" = true ]; then
    COMMAND="$COMMAND --show-reasoning"
fi

if [ "$SHOW_DEFAULT_MODEL" != true ] && [ "$SHOW_AGENT_GRAPH" = true ]; then
