    ...
    for test_date in analysis_dates:
        ...
        for fname, fmask in filters:
            ...
            if len(removed) > 0:
                ...
                print(f"  {fname}: 过滤{len(removed):>4}只 " f"胜率={r_win:.0%} 大涨={r_big:.0%} 均收={r_avg:+.2f}% " f"(含{len(big_removed)}只大涨股)", end="")
        ...
        for threshold_k in [0, 50000, 30000, 10000, 5000]:
            ...
            print(f"    amount>={threshold_k:>6}千元({threshold_k/10000:.0f}万): {len(pool):>4}只 " f"胜率={p_win:.0%} 大涨={len(big_stocks):>3}只({p_big:.0%}) 均收={p_avg:+.2f}%")
