...
def _load():
    ...
    for ff in Path("data/fund_flow_cache/").glob("*.csv"):
        ...
        f[t] = [FundFlowRecord(ticker=t, date=str(r["date"]), close=float(r.get("close", 0) or 0), pct_change=float(r.get("pct_change", 0) or 0), main_net_inflow=float(r.get("main_net_inflow", 0) or 0), main_net_pct=float(r.get("main_net_pct", 0) or 0)) for _, r in df.iterrows()]
    return p, f
    ...
    for today in trading_days:
        ...
        for p in pos:
            ...
            if low <= sp:
                ...
                still.append(p)
        pos = still
        if today not in eset or len(pos) >= _MAX_POSITIONS:
            continue
        slots = _MAX_POSITIONS - len(pos)
        nb = 0
        for ticker, tdf in prices.items():
            if nb >= slots:
                break
            if any(p["ticker"] == ticker for p in pos):
                continue
            r = tdf.index[tdf["date"] == today].tolist()
            if not r or r[0] + 1 >= len(tdf):
                continue
            ti = r[0]
            up = tdf.iloc[: ti + 1].copy()
            up["date"] = pd.to_datetime(up["date"], format="%Y%m%d")
            fu = [r for r in flow.get(ticker, []) if r.date <= today]
            lp = float(up.iloc[-1].get("pct_change", 0) or 0)
            ip = max(lp, 3.0) if lp >= 9.5 else lp
            ...
            if result.hit:
                ...
                nb += 1
        peak = max(peak, nav)
        md = min(md, nav / peak - 1)
    n = len(closed)
    w = sum(1 for p in closed if p["pnl"] > 0)
    rets = [nav for nav in [nav]]
    from pathlib import Path

    return {"nav": nav, "ret": (nav - 1) * 100, "wr": w / n if n else 0, "md": md * 100, "n": n, "sharpe": 0, "wins": w, "exits": {r: sum(1 for p in closed if p["exit_reason"] == r) for r in set(p["exit_reason"] for p in closed)}}
...
def main():
    ...
    print("-" * 56)
    for l, v1, v2 in [("交易数", ra["n"], rb["n"]), ("胜率", f"{ra['wr']:.0%}", f"{rb['wr']:.0%}"), ("总收益", f"{ra['ret']:+.2f}%", f"{rb['ret']:+.2f}%"), ("最大回撤", f"{ra['md']:+.2f}%", f"{rb['md']:+.2f}%"), ("Sharpe", f"{ra['sharpe']:.2f}", f"{rb['sharpe']:.2f}"), ("退出", str(ra["exits"]), str(rb["exits"]))]:
