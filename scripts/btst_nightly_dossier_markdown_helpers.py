...
def append_candidate_pool_recall_corridor_details_markdown(lines: list[str], summary: dict[str, Any]) -> None:
    ...
    if corridor_summary:
        lines.append(f"- corridor_validation_pack_summary: pack_status={corridor_summary.get('pack_status')} primary_validation_ticker={corridor_summary.get('primary_validation_ticker')} promotion_readiness_status={corridor_summary.get('promotion_readiness_status')} parallel_watch_tickers={corridor_summary.get('parallel_watch_tickers')}")
