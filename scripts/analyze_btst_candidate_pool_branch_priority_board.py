...
def analyze_btst_candidate_pool_branch_priority_board(dossier_path: str | Path, lane_objective_support_path: str | Path | None = None) -> dict[str, Any]:
    ...
    for row in branch_queue:
        ...
        execution_priority_rank_hint = 99
        if handoff == "post_gate_liquidity_competition" and prototype_readiness == "shadow_ready_rebucket_signal":
            execution_priority_rank_hint = 1
        elif handoff == "layer_a_liquidity_corridor":
            execution_priority_rank_hint = 2
        else:
            execution_priority_rank_hint = 3 + _readiness_rank(prototype_readiness)
        branch_rows.append(
            {
                ...
                "execution_priority_rank_hint": execution_priority_rank_hint,
    ...
    if structural_leader and objective_leader:
    ...
    if branch_rows:
        ...
        next_3_tasks.append(
            {
                "task_id": str(first.get("task_id") or "candidate_pool_branch_task"),
                "title": f"优先推进 {first.get('priority_handoff')} shadow lane",
                "why_now": str(first.get("evaluation_summary") or ""),
    ...
    next_3_tasks = next_3_tasks[:3]

    recommendation = "当前 candidate-pool recall 的第一优先 lane 应切到 post_gate competition rebucket shadow，其次再按 corridor uplift burden 排序 003036/300720 这类上游流动性修复样本。"
    if branch_rows:
