...
def test_verdict_bias_amplified_includes_amplification_ratio():
    ...
    lines = render_price_verdict(
        universe_ic=0.049, pool_ic=0.176, n_records=7993, n_days=18,
    )
    joined = "\n".join(lines)
    assert "选择偏差" in joined  # bias_amplified 判读
    # amplification 倍数必须出现在判读里 (de-orphan amplification_ratio)
    assert "×" in joined or "x" in joined.lower(), (
        f"判读应含 amplification 倍数 (×), got:\n{joined}"
    )


def test_verdict_real_factor_does_not_claim_bias():
    lines = render_price_verdict(
        universe_ic=0.15, pool_ic=0.176, n_records=5000, n_days=18,
    )
    joined = "\n".join(lines)
...
def test_verdict_mixed_mentions_partial():
    lines = render_price_verdict(
        universe_ic=0.07, pool_ic=0.176, n_records=5000, n_days=18,
    )
    joined = "\n".join(lines)
...
def test_verdict_amplification_none_when_universe_zero():
    ...
    lines = render_price_verdict(
        universe_ic=0.0, pool_ic=0.176, n_records=100, n_days=5,
    )
    joined = "\n".join(lines)
