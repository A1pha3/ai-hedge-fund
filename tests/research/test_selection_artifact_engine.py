...
class _ExplodingWriter:
    def write_for_plan(self, **kwargs):
        raise RuntimeError("writer exploded")


def test_backtest_engine_keeps_plan_when_selection_artifact_writer_raises():
    pipeline_stub = SimpleNamespace(
        base_model_name="MiniMax-M2.7",
        base_model_provider="MiniMax",
        frozen_post_market_plans=None,
    ...
    engine = BacktestEngine(
        ...
        backtest_mode="pipeline",
        pipeline=pipeline_stub,
        selection_artifact_writer=_ExplodingWriter(),
    )
    plan = ExecutionPlan(
        date="20260320",
        portfolio_snapshot={"cash": 100000.0, "positions": {}},
    ...
    assert plan.selection_artifacts["write_status"] == "failed"
    assert "writer exploded" in plan.selection_artifacts["error_message"]