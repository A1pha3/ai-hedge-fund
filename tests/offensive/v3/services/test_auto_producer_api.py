"""Plan 05 Task 5 (RED): AutoProducerApi 能力矩阵 + 信号漏斗 + shadow gate。
...
8. auto shadow gate — OFF/AUTHORITATIVE/BTST_CANARY/默认 None → 抛
   AUTO_PRODUCER_NOT_SHADOW 且不触达 store; SHADOW → 成功。
...
# --------------------------------------------------------------------------
# World: trusted registry (auto + btst 两个 issuer) + 两个服务 + 裸 store
# --------------------------------------------------------------------------


class _World:
    """信任上下文 + 同一 evidence DB 上的 auto/btst 服务与裸仓库."""

    def __init__(
        self,
        tmp_path: Path,
        *,
        auto_fingerprint: str = AUTO_FINGERPRINT,
        btst_fingerprint: str = BTST_FINGERPRINT,
    ) -> None:
        self.clock = _Clock(NOW)
        self.auto_key = Ed25519PrivateKey.generate()
        self.btst_key = Ed25519PrivateKey.generate()
        self.auto_capability = _trust_capability(AUTO_NS)
        self.btst_capability = _trust_capability(BTST_NS)
        ...
        self.raw_auto_repository = EvidenceRepository(
            database_path=self.database_path,
            blob_store=self.blob_store,
            verifier=self.verifier,
            trust_head_provider=self.head_provider,
            issuer_namespace=AUTO_NS,
            clock=self.clock,
        )
        self.raw_btst_repository = EvidenceRepository(
            database_path=self.database_path,
            blob_store=self.blob_store,
            verifier=self.verifier,
            trust_head_provider=self.head_provider,
        ...
        self.auto_service = AutoProducerApi(
            ...
            # world 恒运行于 SHADOW 模式; 非 SHADOW 拒绝由
            # test_shadow_gate_rejects_non_shadow_modes 独立验证。
            runtime_mode_provider=lambda: RuntimeMode.SHADOW,
        )
        self.btst_service = BtstProducerApi(
            database_path=self.database_path,
            blob_store=self.blob_store,
            verifier=self.verifier,
            trust_head_provider=self.head_provider,
            clock=self.clock,
            signer=self.sign_btst,
            behavior_fingerprint=btst_fingerprint,
        )
        self.signed_payloads: dict[str, list[bytes]] = {"auto": [], "btst": []}

    def sign_auto(self, payload: bytes):
        self.signed_payloads["auto"].append(payload)
        return _signed(
            self.auto_key,
            self.auto_capability,
            issuer_id="auto.producer",
    ...
    def sign_btst(self, payload: bytes):
        self.signed_payloads["btst"].append(payload)
        return _signed(
            self.btst_key,
            self.btst_capability,
            issuer_id="btst.producer",
    ...
    def signal_envelope(self, evidence_id: str, *, producer: str, **overrides):
        values = {
            "evidence_id": evidence_id,
            "subject_scope": EvidenceScope.STRATEGY_LINEAGE,
            "subject_producer": producer,
            "family_id": f"{producer}:{SNAPSHOT_ID}",
            "strategy_semver": "0.1.0",
            "behavior_fingerprint": HASH,
            "policy_epoch": 1,
            "execution_version": f"{producer}.funnel.v1",
            ...
            "mode": ExecutionMode.RESEARCH_RECONSTRUCTION,
            "source_authority": f"{producer}.producer",
...
def test_full_funnel_publishes_candidate_and_selected_per_candidate(
    ...
    assert all(r.evidence.subject_producer == "auto" for r in records)
    assert all(r.evidence.family_id == f"auto:{SNAPSHOT_ID}" for r in records)
...
def test_behavior_fingerprint_is_injected_onto_every_envelope(
    ...
    fingerprint_a = "a" * 64
    fingerprint_b = "b" * 64
    world_a = _World(tmp_path / "fp-a", auto_fingerprint=fingerprint_a)
    world_b = _World(tmp_path / "fp-b", auto_fingerprint=fingerprint_b)

    records_a = world_a.auto_service.produce_and_publish(_snapshot())
    records_b = world_b.auto_service.produce_and_publish(_snapshot())
    ...
    assert world_a.signed_payloads["auto"] != world_b.signed_payloads["auto"]
...
def test_signal_envelopes_carry_no_authorization_field(world: _World) -> None:
    ...
    for payload in world.signed_payloads["auto"]:
...
def test_auto_and_btst_evidence_id_spaces_are_disjoint(world: _World) -> None:
    auto_records = world.auto_service.produce_and_publish(_snapshot())
    btst_records = world.btst_service.produce_and_publish(_snapshot())

    assert len(auto_records) == 4 and len(btst_records) == 4
    assert all(r.evidence.subject_producer == "auto" for r in auto_records)
    assert all(r.evidence.subject_producer == "btst" for r in btst_records)
    assert all(r.evidence.family_id == f"auto:{SNAPSHOT_ID}" for r in auto_records)
    assert all(r.evidence.family_id == f"btst:{SNAPSHOT_ID}" for r in btst_records)

    auto_ids = {r.evidence.evidence_id for r in auto_records}
    btst_ids = {r.evidence.evidence_id for r in btst_records}
    assert auto_ids & btst_ids == set()
    # 同 ticker 在两侧产生不同 evidence_id
    for ticker in TICKERS:
        auto_for_ticker = {eid for eid in auto_ids if ticker in eid}
        btst_for_ticker = {eid for eid in btst_ids if ticker in eid}
        assert auto_for_ticker and btst_for_ticker
        assert auto_for_ticker & btst_for_ticker == set()

    # 两侧互不干扰: auto 的 evidence_id 只能从 auto 面读回
    world.clock.advance(seconds=1)
    for evidence_id in auto_ids:
        ...
        assert active is not None
        assert active.evidence.subject_producer == "auto"
    for evidence_id in btst_ids:
        active = world.btst_service.active_signal(
            evidence_id, cutoff=world.clock.now_value
        )
        assert active is not None
        assert active.evidence.subject_producer == "btst"


def test_shadow_gate_rejects_non_shadow_modes(
    world: _World, tmp_path: Path
) -> None:
    for mode in (
        RuntimeMode.OFF,
        RuntimeMode.AUTHORITATIVE,
        RuntimeMode.BTST_CANARY,
    ):
        gated = AutoProducerApi(
            ...
            clock=world.clock,
            signer=world.sign_auto,
            behavior_fingerprint=AUTO_FINGERPRINT,
            runtime_mode_provider=lambda m=mode: m,
        )
        with pytest.raises(AutoProducerApiError) as excinfo:
            gated.produce_and_publish(_snapshot())
        assert excinfo.value.code == AUTO_PRODUCER_NOT_SHADOW

    # fail-closed 默认: 不注入 runtime_mode_provider (None) → 同样拒绝
    defaulted = AutoProducerApi(
        database_path=world.database_path,
        blob_store=world.blob_store,
        verifier=world.verifier,
        trust_head_provider=world.head_provider,
    ...
    with pytest.raises(AutoProducerApiError) as excinfo:
        defaulted.produce_and_publish(_snapshot())
    assert excinfo.value.code == AUTO_PRODUCER_NOT_SHADOW

    # 被拒的调用不触达 store: 无签名 payload, 无提交行
    assert world.signed_payloads["auto"] == []
    with pytest.raises(EvidenceStoreError):
        world.raw_auto_repository.get("auto:never-published")


def test_shadow_mode_publishes_successfully(world: _World) -> None:
    gated = AutoProducerApi(
        database_path=world.database_path,
        blob_store=world.blob_store,
        verifier=world.verifier,
    ...
    records = gated.produce_and_publish(_snapshot())

    assert len(records) == 4
    assert len(world.signed_payloads["auto"]) == 4


def test_active_signal_returns_none_for_unknown_evidence(world: _World) -> None:
    world.clock.advance(seconds=1)
    assert (
        world.auto_service.active_signal(
            "auto:never-published", cutoff=world.clock.now_value
        )
        is None
    )
