...
def test_compute_kelly_size_half_kelly():
    ...
    dist = Distribution(n=60, winrate=0.6, avg_gain=0.20, avg_loss=-0.08, convexity_ratio=3.0, expected_return=0.1, ci_low=0.05, ci_high=0.15, ic=0.08)
...
def test_compute_kelly_size_capped_at_max_pct():
    ...
    dist = Distribution(n=60, winrate=0.7, avg_gain=0.30, avg_loss=-0.05, convexity_ratio=10.0, expected_return=0.2, ci_low=0.15, ci_high=0.25, ic=0.10)
...
def test_compute_kelly_size_correlation_discount_reduces_position():
    ...
    dist = Distribution(n=60, winrate=0.52, avg_gain=0.04, avg_loss=-0.04, convexity_ratio=1.3, expected_return=0.005, ci_low=-0.01, ci_high=0.02, ic=0.06)
...
def test_compute_kelly_size_negative_returns_zero():
    """负期望 (Kelly<0) → 仓位 0。"""
    dist = Distribution(n=60, winrate=0.3, avg_gain=0.10, avg_loss=-0.10, convexity_ratio=0.5, expected_return=-0.04, ci_low=-0.08, ci_high=0.0, ic=-0.05)
    ks = compute_kelly_size(dist)
