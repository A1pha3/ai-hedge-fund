...
def test_inverted_factor_classified_noisy_when_ci_crosses_zero():
    ...
    if t.inversion_ci_low < 0.0 < t.inversion_ci_high:
        assert t.verdict == "inverted_noisy", f"CI 跨 0 ({t.inversion_ci_low:.2f}..{t.inversion_ci_high:.2f}) " f"但 verdict={t.verdict}, 应为 inverted_noisy"
...
def test_render_marks_noisy_inversion():
    ...
    if t.verdict == "inverted_noisy":
        ...
        assert "噪声" in line or "noisy" in line.lower(), f"inverted_noisy 应有噪声标记, got: {line!r}"
...
def test_empty_strategy_keys_is_insufficient_not_ok():
    ...
    assert rep.verdict == "insufficient", f"空 base_contributions 应 insufficient, got verdict={rep.verdict} " f"factors={len(rep.factors)}"
