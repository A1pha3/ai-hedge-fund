...
def _seed_report(dir_path: Path, date_str: str, tickers: list[str]) -> None:
    ...
    """
    recs = [
        {"ticker": t, "name": t, "score_b": round(0.9 - 0.1 * i, 2)}
...
class TestComputeRecommendationStability:
    def test_all_same_top3_is_stable(self, tmp_path: Path) -> None:
        ...
        report = compute_recommendation_stability(
            reports_dir=tmp_path, lookback_days=5, top_n=3
        )
...
class TestRenderStabilityLine:
    def test_insufficient_returns_empty(self) -> None:
        report = RecommendationStabilityReport(
            lookback_days=5, top_n=3, day_count=1, stability_score=None
        )
        assert render_stability_line(report) == ""

    def test_stable_render(self) -> None:
        report = RecommendationStabilityReport(
            lookback_days=5, top_n=3, day_count=5, stability_score=1.0, label="稳定"
        )
    ...
    def test_volatile_render(self) -> None:
        report = RecommendationStabilityReport(
            lookback_days=5, top_n=3, day_count=5, stability_score=0.5, label="波动"
        )
        result = render_stability_line(report)
        assert "50%" in result
        assert "波动" in result
