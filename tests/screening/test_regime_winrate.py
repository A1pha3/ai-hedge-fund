...
class TestFormatCiLabel:
    ...
    def test_ci_level_integer_percent_no_fraction(self) -> None:
        ...
        assert "99.5%" in label, f"ci_level=0.995 should produce '99.5% CI', got {label!r}"
...
class TestRegimeWinrateLineWithBootstrapCi:
    ...
    def test_json_with_ci_appears_in_rendered_line(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        (json_dir / "regime_winrates_recomputed_20260704.json").write_text(json.dumps(payload), encoding="utf-8")
        ...
        assert "平均 +0.6" in line or "平均 0.6%" in line, f"avg_return 0.58 should appear as '平均 +0.6%' in rendered line:\n{line}"

    def test_json_without_ci_still_renders(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestRegimeAdviceNotStale:
    ...
    def test_advice_does_not_hardcode_stale_winrate_numbers(self) -> None:
        ...
        for regime, advice in _REGIME_ADVICE.items():
            ...
            assert "~" not in advice or "%" not in advice, f"{regime} advice hardcodes a stale winrate: {advice!r} — " f"this contradicts the headline winrate when data changes"
    ...
    def test_advice_does_not_make_data_dependent_alpha_claims(self) -> None:
        ...
        for regime, advice in _REGIME_ADVICE.items():
            ...
            assert "无显著 alpha" not in advice, f"{regime} advice claims '无显著 alpha' — this is data-dependent " f"and goes stale when winrate CI moves above 50%"
...
class TestRegimeWinrateHorizonDisclosure:
    ...
    def test_headline_line_labels_horizon_as_t30(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert "T+30" in line, f"headline winrate is T+30 口径 but render doesn't label it — " f"operator next to a BUY (T+5/T+10) can mistake it for the " f"BUY-horizon winrate. line={line!r}"

    def test_multihorizon_line_surfaces_buy_horizon_winrate(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
