...
def test_write_summary_atomic_crash_preserves_prior(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(mod, "_SUMMARY", {
        "session_id": "test", "totals": {"attempts": 0, "total_duration_ms": 0.0}, "buckets": {}
    })
    ...
    assert raw.strip(), (
        "prior LLM 指标汇总 must not be truncated-empty after a crashed write — "
        "non-atomic open('w') truncates immediately (R88 corrupt-summary CRASH vector root cause)"
    )
