...
def test_analyze_layer_b_boundary_failures_aggregates_boundary_score_failures(tmp_path):
    ...
    assert analysis["candidate_source_counts"] == {"layer_b_boundary": 1, "short_trade_boundary": 1}
...
def test_analyze_layer_b_boundary_failures_can_filter_trade_dates(tmp_path):
    report_dir = tmp_path / "report"
    selection_root = report_dir / "selection_artifacts"
    (selection_root / "2026-03-10").mkdir(parents=True)
    (selection_root / "2026-03-11").mkdir(parents=True)
                    ...
                    "000001": {
                        ...
                        "short_trade": {
                            ...
                            "metrics_payload": {
                                "score_b": 0.3,
                                "score_final": 0.1,
                                "weighted_positive_contributions": {"trend_acceleration": 0.05},
                                "weighted_negative_contributions": {"overhead_supply_penalty": 0.04},
                            },
                        },
                    }
                },
                    ...
                    "000002": {
                        ...
                        "short_trade": {
                            ...
                            "metrics_payload": {
                                "score_b": 0.35,
                                "score_final": 0.2,
                                "weighted_positive_contributions": {"trend_acceleration": 0.06},
                                "weighted_negative_contributions": {"overhead_supply_penalty": 0.03},
                            },
                        },
                    }
                },
    ...
    analysis = analyze_layer_b_boundary_failures(report_dir, trade_dates={"2026-03-11"})

    assert analysis["trade_dates_filter"] == ["2026-03-11"]
    assert analysis["trade_day_count"] == 1
    assert analysis["layer_b_boundary_rejected_count"] == 1
