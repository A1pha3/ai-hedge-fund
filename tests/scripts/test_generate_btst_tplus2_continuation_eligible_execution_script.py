...
def test_generate_btst_tplus2_continuation_eligible_execution_threads_payload(tmp_path: Path) -> None:
    lane_rulepack_path = tmp_path / "lane_rulepack.json"
    watchlist_execution_path = tmp_path / "watchlist_execution.json"
    eligible_gate_path = tmp_path / "eligible_gate.json"
    ...
    assert analysis["execution_verdict"] == "eligible_extension_applied"
    assert analysis["raw_eligible_tickers"] == ["600988"]
    assert analysis["effective_eligible_tickers"] == ["600988", "300505"]
    assert analysis["added_eligible_tickers"] == ["300505"]
    assert analysis["adopted_eligible_row"]["ticker"] == "300505"
    ...
    assert analysis["source_reports"] == {
        ...
        "watchlist_execution": str(watchlist_execution_path.expanduser().resolve()),
        "eligible_gate": str(eligible_gate_path.expanduser().resolve()),
    }
