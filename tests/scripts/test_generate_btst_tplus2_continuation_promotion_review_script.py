...
def test_generate_btst_tplus2_continuation_promotion_review_allows_near_cluster_focus(tmp_path: Path) -> None:
    queue_path = tmp_path / "queue.json"
    focus_dossier_path = tmp_path / "focus.json"
    watch_dossier_path = tmp_path / "watch.json"

    queue_path.write_text(json.dumps({"focus_candidate": {"ticker": "300683"}}), encoding="utf-8")
    focus_dossier_path.write_text(
        json.dumps(
            {
                ...
                "tier_focus_surface_summary": {
                    ...
                    "t_plus_2_close_positive_rate": 1.0,
                    "t_plus_2_close_return_distribution": {"mean": 0.08},
                },
            }
        ),
        encoding="utf-8",
    ...
    assert analysis["promotion_review_verdict"] == "watch_review_ready"
    assert "focus_not_observation_candidate" not in analysis["promotion_blockers"]
