...
def test_generate_btst_tplus2_continuation_execution_overlay_threads_payload(tmp_path: Path) -> None:
    eligible_execution_path = tmp_path / "eligible_execution.json"
    execution_gate_path = tmp_path / "execution_gate.json"
    ...
    assert analysis["execution_verdict"] == "execution_candidate_applied"
    assert analysis["effective_execution_candidates"] == ["300505"]
    assert analysis["added_execution_candidates"] == ["300505"]
    assert analysis["adopted_execution_row"]["ticker"] == "300505"
    ...
    assert analysis["source_reports"] == {
        ...
        "execution_gate": str(execution_gate_path.expanduser().resolve()),
    }
