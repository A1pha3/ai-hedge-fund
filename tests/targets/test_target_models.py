    ...
    return {
        ...
        "strategy_signals": {
            "trend": _make_signal(
                ...
                100.0,
                sub_factors={
                    "momentum": {"direction": 1, "confidence": 35.0, "completeness": 1.0},
                    "adx_strength": {"direction": 1, "confidence": 60.0, "completeness": 1.0},
                    "ema_alignment": {"direction": 1, "confidence": 100.0, "completeness": 1.0},
                    "volatility": {"direction": 1, "confidence": 55.0, "completeness": 1.0},
            ...
            "mean_reversion": _make_signal(0, 0.0).model_dump(mode="json"),
            "fundamental": _make_signal(1, 70.0).model_dump(mode="json"),
        },
        "agent_contribution_summary": {"cohort_contributions": {"analyst": 0.12, "investor": 0.02}},
    }
...
@contextmanager
def _use_short_trade_target_profile_proxy(*, base_profile_name: str, **extra_fields: float | bool | str):
    base_profile = build_short_trade_target_profile(base_profile_name)
    proxy_payload = vars(base_profile).copy()
    proxy_payload.update(extra_fields)
    proxy_profile = SimpleNamespace(**proxy_payload)
    active_token = short_trade_profiles_module._ACTIVE_SHORT_TRADE_TARGET_PROFILE.set(proxy_profile)
    context_token = short_trade_profiles_module._SHORT_TRADE_TARGET_PROFILE_CONTEXT_ACTIVE.set(True)
    try:
        yield proxy_profile
    finally:
        short_trade_profiles_module._SHORT_TRADE_TARGET_PROFILE_CONTEXT_ACTIVE.reset(context_token)
        short_trade_profiles_module._ACTIVE_SHORT_TRADE_TARGET_PROFILE.reset(active_token)


def test_watchlist_filter_diagnostics_selected_only_shrink_profile_contract() -> None:
    with pytest.raises(ValueError, match="Unknown short trade target profile: trend_continuation_strength_v3"):
        build_short_trade_target_profile("trend_continuation_strength_v3")
...
def test_watchlist_filter_diagnostics_selected_only_shrink_downgrades_selected_to_near_miss() -> None:
    ...
    with _use_short_trade_target_profile_proxy(
        base_profile_name="trend_continuation_strength_v2",
        name="trend_continuation_strength_v2_selected_only_shrink_probe",
        ...
        watchlist_filter_diagnostics_selected_only_shrink_trend_acceleration_max=0.61,
        watchlist_filter_diagnostics_selected_only_shrink_close_strength_max=0.80,
    ):
    ...
    assert baseline_result.trend_acceleration <= 0.61
    assert baseline_result.close_strength <= 0.80
...
def test_short_trade_profiles_define_ordered_governance_envelopes() -> None:
    ...
    assert default_profile.selected_rank_cap_relief_score_margin_min == 0.0
    assert default_profile.selected_rank_cap_relief_rank_buffer == 0
    assert default_profile.selected_rank_cap_relief_rank_buffer_ratio == 0.0
    assert default_profile.selected_rank_cap_relief_sector_resonance_min == 0.0
    assert default_profile.selected_rank_cap_relief_close_strength_max == 1.0
    assert default_profile.selected_rank_cap_relief_require_confirmed_breakout is False
    ...
    assert default_profile.catalyst_theme_source_specific_rank_cap_trend_acceleration_min == 0.0
    assert default_profile.catalyst_theme_source_specific_rank_cap_sector_resonance_min == 0.0
    ...
    assert btst_precision_profile.select_threshold == 0.40
    assert btst_precision_profile.near_miss_threshold == 0.30
    assert btst_precision_profile.selected_rank_cap_ratio == 0.08
    ...
    assert catalyst_research_rank_probe.catalyst_theme_source_specific_rank_cap_trend_acceleration_min == 0.78
    assert catalyst_research_rank_probe.catalyst_theme_source_specific_rank_cap_sector_resonance_min == 0.12
...
def test_short_trade_rank_threshold_tightening_raises_thresholds_for_deep_rank_entries() -> None:
    ...
    assert deep_rank_result.effective_select_threshold == pytest.approx(0.42, abs=1e-4)
    assert deep_rank_result.effective_near_miss_threshold == pytest.approx(0.36, abs=1e-4)
...
def test_short_trade_rank_decision_cap_soft_relief_preserves_high_quality_selected() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    ...
    with use_short_trade_target_profile(
        ...
        overrides={
            "selected_rank_cap": 8,
            "near_miss_rank_cap": 25,
            "selected_rank_cap_relief_score_margin_min": 0.10,
    ...
    assert cap_state["selected_cap_exceeded"] is True
    assert cap_state["selected_cap_exceeded_effective"] is False
    assert cap_state["selected_cap_soft_relief_applied"] is True
    assert cap_state["selected_rank_cap_relief_score_pass"] is True
    assert cap_state["selected_rank_cap_relief_within_buffer"] is True


def test_short_trade_rank_decision_cap_soft_relief_does_not_rescue_weak_over_cap_entries() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    ...
    with use_short_trade_target_profile(
        ...
        overrides={
            "selected_rank_cap": 8,
            "near_miss_rank_cap": 15,
            "selected_rank_cap_relief_score_margin_min": 0.20,
            "selected_rank_cap_relief_rank_buffer": 20,
        },
    ):
        deep_rank_result = evaluate_short_trade_rejected_target(
    ...
    assert deep_rank_result.decision == "rejected"
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert "selected_rank_cap_exceeded" in deep_rank_result.rejection_reasons
    assert cap_state["selected_cap_exceeded_effective"] is True
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_score_pass"] is False


def test_short_trade_rank_decision_cap_soft_relief_respects_t_plus_2_guard() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    ...
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_require_t_plus_2_candidate"] is True
...
def test_catalyst_research_rank_probe_releases_research_candidate_from_source_cap_pressure() -> None:
    ...
    entry["candidate_reason_codes"] = list(entry["reasons"])
    entry["strategy_signals"]["trend"]["sub_factors"]["momentum"]["confidence"] = 95.0
    entry["strategy_signals"]["trend"]["sub_factors"]["adx_strength"]["confidence"] = 88.0
    entry["strategy_signals"]["trend"]["sub_factors"]["ema_alignment"]["confidence"] = 100.0
    entry["agent_contribution_summary"] = {"cohort_contributions": {"analyst": 0.20, "investor": 0.10}}
    ...
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_trend_acceleration_min"] == pytest.approx(0.78)
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_sector_resonance_min"] == pytest.approx(0.12)
...
def test_catalyst_research_rank_probe_does_not_release_research_candidate_with_weak_trend_acceleration() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    entry["ticker"] = "301205"
    entry["reasons"] = [
        "catalyst_theme_candidate_score_ranked",
    ...
    entry["candidate_reason_codes"] = list(entry["reasons"])
    entry["strategy_signals"]["trend"]["sub_factors"]["momentum"]["confidence"] = 78.0
    entry["strategy_signals"]["trend"]["sub_factors"]["adx_strength"]["confidence"] = 72.0
    entry["strategy_signals"]["trend"]["sub_factors"]["ema_alignment"]["confidence"] = 85.0
    entry["agent_contribution_summary"] = {"cohort_contributions": {"analyst": 0.02, "investor": 0.01}}
    ...
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_trend_acceleration_pass"] is False
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_sector_resonance_pass"] is True


def test_catalyst_research_rank_probe_does_not_release_research_candidate_with_weak_sector_resonance() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    entry["ticker"] = "600522"
    entry["score_c"] = -0.7
    ...
    entry["candidate_reason_codes"] = list(entry["reasons"])
    entry["strategy_signals"]["trend"]["sub_factors"]["momentum"]["confidence"] = 90.0
    entry["strategy_signals"]["trend"]["sub_factors"]["adx_strength"]["confidence"] = 85.0
    entry["strategy_signals"]["trend"]["sub_factors"]["ema_alignment"]["confidence"] = 95.0
    ...
    assert probe_cap_state["catalyst_theme_source_specific_caps_enabled"] is False
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_trend_acceleration_pass"] is True
    assert probe_cap_state["catalyst_theme_source_specific_rank_cap_sector_resonance_pass"] is False
...
def test_short_trade_rank_decision_cap_soft_relief_respects_sector_resonance_floor() -> None:
    ...
    with use_short_trade_target_profile(
        ...
        overrides={
            ...
            "selected_rank_cap_relief_score_margin_min": 0.0,
            "selected_rank_cap_relief_rank_buffer": 20,
            "selected_rank_cap_relief_sector_resonance_min": 0.20,
        },
    ):
        deep_rank_result = evaluate_short_trade_rejected_target(
            trade_date="20260328",
    ...
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_sector_resonance_min"] == 0.2
    assert cap_state["selected_rank_cap_relief_sector_resonance_pass"] is False


def test_short_trade_rank_decision_cap_soft_relief_respects_close_strength_cap() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    ...
    with use_short_trade_target_profile(
        ...
        overrides={
            ...
            "selected_rank_cap_relief_score_margin_min": 0.0,
            "selected_rank_cap_relief_rank_buffer": 20,
            "selected_rank_cap_relief_close_strength_max": 0.80,
        },
    ):
        deep_rank_result = evaluate_short_trade_rejected_target(
            trade_date="20260328",
    ...
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_close_strength_max"] == 0.8
    assert cap_state["selected_rank_cap_relief_close_strength_pass"] is False


def test_short_trade_rank_decision_cap_soft_relief_respects_risk_off_market_gate() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    entry["market_state"] = {
        "breadth_ratio": 0.40,
        "position_scale": 0.70,
    ...
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_market_risk_level"] == "risk_off"
    assert cap_state["selected_rank_cap_relief_market_risk_pass"] is False


def test_short_trade_rank_decision_cap_soft_relief_respects_crisis_market_gate() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    entry["market_state"] = {
        "breadth_ratio": 0.32,
        "position_scale": 0.50,
    ...
    assert deep_rank_result.gate_status.get("rank") == "selected_cap_exceeded"
    assert cap_state["selected_cap_soft_relief_applied"] is False
    assert cap_state["selected_rank_cap_relief_market_risk_level"] == "crisis"
    assert cap_state["selected_rank_cap_relief_market_risk_pass"] is False


def test_short_trade_rank_decision_cap_soft_relief_uses_volatility_fallback_when_market_state_missing() -> None:
    entry = _make_catalyst_theme_short_trade_carryover_entry()
    entry["market_state"] = {}
    entry["strategy_signals"]["trend"]["sub_factors"]["volatility"]["metrics"] = {
        "volatility_regime": 1.26,
        "atr_ratio": 0.098,
    }
    ...
    cap_state = deep_rank_result.metrics_payload["thresholds"]["rank_decision_cap"]
    assert deep_rank_result.decision == "near_miss"
    assert cap_state["selected_rank_cap_relief_market_risk_source"] == "volatility_fallback"
    assert cap_state["selected_rank_cap_relief_market_risk_level"] in {"risk_off", "crisis"}
    assert cap_state["selected_rank_cap_relief_market_risk_pass"] is False
...
def test_short_trade_market_state_threshold_adjustment_tightens_crisis_regime() -> None:
    ...
    assert crisis_result.effective_select_threshold == pytest.approx(0.43, abs=1e-4)
    assert crisis_result.effective_near_miss_threshold == pytest.approx(0.36, abs=1e-4)
...
def test_historical_execution_relief_promotes_strong_close_continuation_frontier_to_selected() -> None:
    ...
    assert round(relief_result.score_target, 4) == pytest.approx(0.5746, abs=1e-4)
    assert baseline_result.decision in {"near_miss", "selected"}
...
def test_upstream_shadow_catalyst_relief_promotes_strong_recalled_shadow_to_near_miss() -> None:
    ...
    assert baseline_result.decision in {"rejected", "near_miss", "selected"}
    assert round(baseline_result.score_target, 4) == pytest.approx(0.4304, abs=1e-4)
    assert baseline_result.metrics_payload["catalyst_freshness"] == 0.0
    assert baseline_result.metrics_payload["effective_catalyst_freshness"] == 0.0
    assert relief_result.decision in {"near_miss", "selected"}
    assert round(relief_result.score_target, 4) == pytest.approx(0.5478, abs=1e-4)
...
def test_upstream_shadow_catalyst_relief_can_promote_post_gate_shadow_to_selected() -> None:
    ...
    assert round(result.score_target, 4) == pytest.approx(0.5478, abs=1e-4)
    assert result.metrics_payload["thresholds"]["effective_select_threshold"] == pytest.approx(0.40, abs=1e-4)
    assert result.metrics_payload["thresholds"]["upstream_shadow_catalyst_relief_select_threshold_override"] == 0.45
    assert result.explainability_payload["upstream_shadow_catalyst_relief"]["effective_select_threshold"] == pytest.approx(0.40, abs=1e-4)
...
def test_catalyst_theme_short_trade_carryover_keeps_profitability_hard_cliff_sample_rejected() -> None:
    ...
    assert result.decision in {"near_miss", "selected", "rejected"}
    assert result.metrics_payload["profitability_hard_cliff"] is True
    assert result.metrics_payload["upstream_shadow_catalyst_relief_applied"] is False
    assert result.metrics_payload["upstream_shadow_catalyst_relief_gate_hits"]["no_profitability_hard_cliff"] is False
...
def test_catalyst_theme_short_trade_carryover_uses_historical_execution_relief_for_strong_close_continuation() -> None:
    ...
    assert result.metrics_payload["thresholds"]["effective_select_threshold"] == pytest.approx(0.40, abs=1e-4)
    assert result.explainability_payload["historical_execution_relief"]["effective_select_threshold"] == pytest.approx(0.40, abs=1e-4)
...
def test_visibility_gap_continuation_relief_promotes_selected_visibility_gap_shadow_to_near_miss() -> None:
    ...
    assert result.decision in {"near_miss", "selected"}
    assert round(result.score_target, 4) == pytest.approx(0.4657, abs=1e-4)
...
def test_visibility_gap_continuation_relief_requires_relaxed_band_when_profile_demands_it() -> None:
    ...
    assert result.decision in {"rejected", "near_miss", "selected"}
    assert round(result.score_target, 4) == pytest.approx(0.4326, abs=1e-4)
    assert result.metrics_payload["thresholds"]["near_miss_threshold"] == 0.34
    assert result.metrics_payload["visibility_gap_continuation_relief"]["applied"] is False
