...
def test_extract_metrics_from_keyed_report_map(tmp_path):
    active, source = _make_active_and_source(tmp_path)
...
def test_main_accepts_cli_argv_and_writes_outputs(tmp_path):
    active, source = _make_active_and_source(tmp_path)
    out_json = tmp_path / "bridge_cli.json"
    out_md = tmp_path / "bridge_cli.md"
