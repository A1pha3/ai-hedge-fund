...
description: 在本仓库中处理中文 BTST 次日计划、全套文档、执行清单、预警池、交集票高亮、方案 A 观察期目录、BTST 选股策略优化、profile 对照、交易前决策卡等请求时触发。典型触发语义：BTST + 明天/次日/某日收盘数据 + 交易计划|全套文档|执行清单|通俗说明|预警文档|交集票|early-runner|scheme_a|保守|激进|profile 对照|决策卡。
---
...
- **Alpha（统计与赔率）**
  - 只对“这批候选是否具备足够的胜率稳定性与赔率质量”负责。
  - 必须抽取并写清：样本量、区间/收缩式谨慎提示、盈亏比质量、胜率与赔率是否分化。
  - 当 artifacts 只能支持点估计、不能支持稳定 edge 时，明确写“证据不足”或“artifacts not available”，不能把点估计包装成硬优势。
- **Beta（执行与降级）**
  - 只对“明天怎么下、何时确认、何时取消、何时降级/回补”负责。
  - 必须把触发、取消、降级、回补、时段、仓位/成本约束写成可执行语句或表格。
  - 不得用“看盘再说”“强就上”这类空泛表述替代执行 contract。
- **Gamma（市场 / 板块 / 赚钱效应）**
  - 只对“当前大盘、板块、赚钱效应是否支持更激进或更保守的执行姿态”负责。
  - 必须优先复述 artifacts 已给出的 market gate、板块强弱、题材集中度、赚钱效应/情绪语义。
  - 如果当前 run 没有支持字段，只能写“相关 artifacts not available”，不能补通用盘面评论。

## 默认优化优先级

1. **胜率 / 赔率严谨性优先（Alpha first）**
2. **执行细节具体性第二（Beta second）**
3. **市场 / 板块 / 赚钱效应语境第三（Gamma third）**

只要三者冲突，先保住命中率与盈亏比质量，再补执行细节，最后才扩写市场语境。
...
5. 读取当前 artifacts。
   ...
   - 先看 `operator_summary.json` 的运行状态和决策阶段；再用 `session_summary.json` 作为多智能体 provenance 与 `btst_followup` 路径真源。
   ...
   - 其余读取优先级、缺字段降级、Alpha/Beta/Gamma 证据抽取，统一按 `references/artifact-reading.md` 执行，不在这里重复展开。
...
6. 先做选股分层，再写最终文档。
   ...
   uv run python scripts/generate_btst_doc_bundle.py \
     ...
     --output-dir outputs/YYYYMM/<signal_dir> \
     --strategy-thresholds-config config/btst_strategy_thresholds.json \
     --no-refresh-early-runner
   ```

   - 其中 `<signal_dir>` 在方案 A 观察期使用 `YYYYMMDD_scheme_a`，观察期通过后才切回 `YYYYMMDD`。
   ...
   - 核心文档内必须显式落地 `references/final-doc-spec.md` 规定的三张硬区块，标题名称保持完全一致。
   - 决策卡优先看 4 个字段：
     - 推荐 profile
     - 交集票差异
   ...
   - **硬降级 gate：**
     - 样本质量弱、赔率质量弱、market gate 未放行、或板块/赚钱效应语境偏弱时，必须降级候选，不能维持正式主票口径。
     - 具体的 Alpha/Beta/Gamma 降级写法与硬区块标题，统一按 `references/final-doc-spec.md` 执行。
   - 只有在**用户明确接受精简**，且当前不是必须保留 early-runner 预警输出的方案 A 场景时，才可以只生成 `5` 份核心文档并使用 `--core-only`。
...
7. 交付前验证。
   ...
   - Alpha / Beta / Gamma 三条线都要有交付证据；具体验收字段与失败降级口径，统一按 `references/final-doc-spec.md` 与 `references/final-response-template.md` 执行。
...
- 当样本质量、赔率质量、market gate 或板块/赚钱效应语境偏弱时，必须降级候选，不能维持正式主票口径。
- 不能把高胜率但低赔率、或低样本下的点估计，包装成稳定 alpha。
- 不能把缺少 artifacts 支持的板块情绪、赚钱效应、执行触发/取消规则写成既成事实。
...
- 一般字段缺失时直接省略，不猜；但对 `胜率/赔率诊断卡（Alpha）`、`执行触发/取消/升级/降级矩阵（Beta）`、`大盘-板块-赚钱效应环境卡（Gamma）` 这三类硬区块，若当前 artifacts 不支持，必须显式写 `artifacts not available` 或 `context weak`，不能直接消失。
- 需要交互输入时，向用户要答案后继续。
...
- 如果已经生成双 profile 对照和交易前决策卡，最终回复不能只丢路径，必须明确说今天更偏 `conservative` 还是 `aggressive`；并且始终保留 `1 条 Alpha 统计质量理由 + 1 条 Gamma 市场/gate 理由` 作为主骨架。交集票 / only early-runner / second-entry 只能作为补充强因子，不能替代这条主骨架。
...
- 在发送最终中文回复前，加载 `references/final-response-template.md`。
...
- `operator_summary.json`、统一入口、ONE-PAGER、profile compare scope、最终回复格式的细则，统一以 lazy-loaded reference 文件为准：
  - 读取优先级与 failed/degraded 流程：`references/artifact-reading.md`
  - 文档结构、硬区块标题、profile compare 边界：`references/final-doc-spec.md`
  - 决策卡驱动的最终回复顺序与理由口径：`references/final-response-template.md`
