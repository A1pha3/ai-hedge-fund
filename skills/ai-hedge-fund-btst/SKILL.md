...
description: 在本仓库中处理中文 BTST 次日计划、全套文档、执行清单、预警池、交集票高亮、方案 A 观察期目录、BTST 选股策略优化、profile 对照、交易前决策卡等请求时触发。典型触发语义：BTST + 明天/次日/某日收盘数据 + 交易计划|全套文档|执行清单|通俗说明|预警文档|交集票|early-runner|scheme_a|保守|激进|profile 对照|决策卡。核心职责：运行规则版与 short_trade_only 多智能体链路，优先通过 generate_btst_doc_bundle.py 生成主文档，并按正式执行层、交集优先复审层、补充复审层、回补机会层四层输出；在需要保守/激进取舍时额外生成双 profile 文档包对照与交易前决策卡。
---
...
- 仓库默认策略阈值真源是 `config/btst_strategy_thresholds.json`；除非用户明确要求临时实验参数，否则文档流、复盘流、策略建议都要沿用这份配置。
- 当用户明确要求比较风格、优化胜率/盈亏比，或当前需要在 `conservative / aggressive` 之间做执行取舍时，优先补一份双 profile 对照摘要和交易前决策卡。
- 所有输出必须把候选明确拆成 `4` 层：
  - 正式执行层
  - 交集优先复审层
  - 补充复审层
...
6. 先做选股分层，再写最终文档。
   ...
   - 起草前默认读取 `config/btst_strategy_thresholds.json`，把它当作 BTST 四层选股、观察期 gate 和建议阈值的共享基线。
   - 优先使用统一入口：
   ...
   uv run python scripts/generate_btst_doc_bundle.py \
     ...
     --strategy-thresholds-config config/btst_strategy_thresholds.json \
   ...
   - 当请求包含“保守还是激进”“哪套更适合今天”“比较两套 profile”“提高胜率和盈亏比应该偏哪边”这类语义时，优先运行：

   ```bash
   uv run python scripts/generate_btst_doc_bundle.py \
     --signal-date YYYYMMDD \
     --compare-profiles conservative aggressive \
     --output-dir outputs/YYYYMM/YYYYMMDD_profile_compare
   ```

   - 这一步会额外产出：
     - 双 profile 文档包对照 JSON / Markdown
     - `BTST 交易前决策卡` JSON / Markdown
   - 如果决策卡已生成，要把同一套“今日执行倾向”结论桥接进：
     - `BTST-YYYYMMDD.md`
     - `BTST-LLM-YYYYMMDD.md`
   - 最终中文回复也必须沿用同一句结论口径，避免文档说偏保守、回答却说偏激进。
   - 决策卡优先看 4 个字段：
     - 推荐 profile
     - 交集票差异
     - only early-runner 压力
     - second-entry 干扰
   - 状态解释必须固定：
     - `exact`：允许交集票进入优先复审层。
...
- 如果已经生成双 profile 对照和交易前决策卡，最终回复不能只丢路径，必须明确说今天更偏 `conservative` 还是 `aggressive`，以及理由来自交集票 / only early-runner / second-entry 哪一项。
- 如果决策卡已经桥接进主文档，最终回复必须与主文档里的“今日执行倾向”保持完全同口径，不能一边偏保守一边偏激进。
...
- `config/btst_strategy_thresholds.json` 定义默认策略阈值；如果用户没有明确要求试验值，就不要偏离它。

## Lazy loading

- 在读取 run outputs 前，加载 `references/artifact-reading.md`。
- 在起草最终文档前，加载 `references/final-doc-spec.md`。
- 在发送最终中文回复前，如果已经生成交易前决策卡或双 profile 对照，加载 `references/final-response-template.md`。
...
## 来源优先级（P0D 2026-06-04 更新）

读取运行产物时，按以下优先级使用：

1. **`operator_summary.json`**：每次运行的版本化派生读取入口，压缩了运行状态、路径索引和操作者视图。优先检查此文件是否存在且 `schema_version` 支持。
   - 如果 `summary_status` 为 `degraded` 或 `failed`，必须先向用户说明降级原因。
   - 如果此文件不存在或 `schema_version` 不匹配，回退到以下 artifact-reading 流程。
2. **`session_summary.json`**：继续负责多智能体路径与 optimization provenance。
3. **execution artifacts**：继续负责正式执行语义。
4. **early-runner artifacts**：负责观察、确认和 runtime 补充语义。
5. **outcome ledger**（P1）：只负责历史结果和校准。

**兼容策略：**
- 新摘要不存在或 schema 不支持时，回退到当前 artifact-reading 流程。
- 新摘要为 `degraded / failed` 时，必须先说明降级原因。
- 不得依据 `exact` 或 actionability 推导"增量 alpha"。
- 不得让 `operator_summary.json` 覆盖 canonical execution contract。

## 统一入口（P0D 2026-06-04 新增）

当 `operator_summary.json` 不存在时，可以通过统一入口一次性生成：
...
参数说明：
- `--reuse-existing`：默认模式，复用校验通过的 artifacts。
- `--refresh-early-runner`：显式刷新 early-runner（整次运行最多刷新一次）。
- `--with-profile-compare`：可选生成 compare。
- `--dry-run`：只输出计划执行步骤，不写文件。
- `--resume`：从上次失败步骤继续。

此入口会原子写入 `operator_summary.json` 和 `BTST-YYYYMMDD-ONE-PAGER.md`。即使运行失败，仍会写 `failed` 摘要。

## ONE-PAGER（P0D 2026-06-04 新增）

`BTST-YYYYMMDD-ONE-PAGER.md` 从已验证的 `operator_summary.json` 渲染，不复读多个 latest JSON。固定回答：
1. 当前 `decision_phase` 和数据截止时间。
2. 今天是否允许正式交易，以及 canonical `report_mode`。
3. 正式入选票、当前可下单票、仅确认票分别是谁。
4. 第一取消条件。
5. market gate 与 gate enforcement。
6. early-runner 的日期对齐、新鲜度、point-in-time 安全和 actionability。
7. 当日增量贡献计数。
8. 历史增量证据；不足时明确写"证据不足"，不能写"无增量价值"。

`post_close_plan` 阶段的 ONE-PAGER 不包含 T+1 确认结果、成交状态或已实现收益。

## profile 对比口径（P0B 2026-06-04 更新）

- 当前 profile 对比的 `comparison_scope` 固定为 `doc_bundle_rendering`，即 profile 只改变文档展示阈值。
- `effective_decision_diff` 为 `False` 时，不得把文档渲染差异描述成已验证的策略优势。
- 当决策卡 `dominant_reason_type` 为 `no_effective_profile_diff` 时，只能写"默认采用 conservative 做风控基线"。
- 真正让 profile 进入上游筛选链路的工作属于 P2，当前不得在回复中暗示已完成。

## 完成时回复

- 如果生成了交易前决策卡或双 profile 对照，顶部摘要必须固定顺序：
  - `今日执行倾向`
  - `核心理由`
  - `文件位置`
  - `主线摘要`
  - `方案 A 状态`（仅在方案 A 激活时保留）
- 说明文件保存到了哪里。
- 用一小段话总结规则版主线和多智能体主线。
- 明确说明本次是否有手工干预。
- 如果生成了交易前决策卡，先给出一句结论：
  - 今天更偏 `conservative` 还是 `aggressive`
  - 最核心的 1-2 个理由是什么
- 如果规则版文档和 LLM 文档都已经写入“今日执行倾向”，最终回复复用同一句结论，不重新发明另一套说法。
- `文件位置` 必须优先列：
  - 决策卡
  - 规则版
  - 多智能体版
