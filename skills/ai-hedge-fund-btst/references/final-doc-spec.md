...
| BTST-YYYYMMDD-EXEC-CHECKLIST.md | Next-morning execution checklist | opening principles, 09:20-09:25, 09:25-09:35, 09:35-10:00, 10:00 onward, intraday discipline, actual execution order, actual profile_name when present, whether overrides were applied, provenance source from session_summary.json when explicitly available, explicit fallback reason when optimization_profile_resolution.mode is default_fallback | multi-agent artifacts first; rule names only as peripheral observation unless promoted by the LLM plan |
