...
- `moving_block_mean_difference()` first equal-weights trade differences within signal date,
  ...
  calendar. It samples non-empty blocks with a deterministic NumPy generator and reports the
  95% percentile interval plus candidate, usable, empty, and sampled-block counts.
...
- `summarize_paired_results()` reports trade and signal-day counts, greedy non-overlapping
  signal-to-latest-exit windows, paired mean/median/worst-decile/downside-tail differences,
  per-arm mean/median/tail returns, holding sessions, exit reasons, coverage, and covered versus
  missing-group legacy means.
- Replay rows retain actual holding sessions and an MFE diagnostic from audited daily highs.
  Daily highs are explicitly marked non-executable. `give_up = MFE - net_return` is reported
...
5. Additional signal-day aggregation and sparse/empty-block coverage brought the focused module
   to **84 passed**.
6. Research plus offensive verification: **727 passed**.
...
- challenger positive-MFE diagnostic denominator: **79**; mean capture ratio **-0.6246** and
  mean give-up **11.8759 percentage points**. The capture value can be negative because a trade
