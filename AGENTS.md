...
BTST regime 前向配对影子试验（Tasks 1–14，2026-08-10 全部实现、v3 全套绿）——**纯前向测量系统，无任何真实发单路径**。两臂（Champion=`IGNORE` 现行生产策略 / Challenger=`NORMAL_ONLY` regime-gate 候选）共享一次冻结 trusted clock、一条 canonical regime observation（cutoff 前）、一次 producer run、一个 capital snapshot；各自落入隔离且守恒的 `DAILY_BAR_PROXY` ledger。契约切变：`PolicySnapshot` schema-major 2（typed `runtime_mode`/`btst_regime_admission_mode`，两臂须绑不同 policy fingerprint，store 拒 `policy_binding_duplicate`）+ `ShadowDecision` schema-major 3（frozen `execution_authority="NONE"`，adapter 每次 capital 写前复验；`shadow_policy_binding` 按哈希绑 sealed policy；`domain_hash` 信封接受 {2,3}，LegacyShadowDecisionV2 只读保历史字节）。原子 pair commit 是副作用边界——崩溃重放精确验证既有 pair、绝不重算替代提案（`shadow_proxy_protocol_breach` latch 同 id 不同内容的 divergent 重放）。共享 `settle_proxy_open` core + capital kernel 经济学：shadow adapter 与授权 proxy 同一经济学，差异仅在 provenance/authority。Task 11 薄 `ForwardPairedTrialRunner`（冻结→bundle 验证→enrolled session→regime 读→producer once→两臂 decide_shadow once→commit_pair→reserve）+ 终态 `SessionSpine`（非 cancel 状态幂等终态、`SESSION_CANCELLED` 需签名 calendar revision）。Task 12 `ForwardTrialReplayEngine` 确定性重放 current-cost（逐字复现 official）与 2×-slippage（仅成本模型不同）。Task 13 frozen paired evaluator（excess log growth、单边 95% LCB、ESS、CDaR、chronological outer fold）+ 可删除 `TrialAssessmentProjection`（只引用 hash + 计算门，headline 最多 `INACTIVE_PROMOTION_CANDIDATE`，永不序列化 broker/authority 载荷）。Task 14：shadow CLI（`src/cli/v3_regime_trial.py`+`scripts/v3_regime_trial.py`，validate/decide-session/advance-session/assess；根目录 traversal/symlink/缺失守卫、无 override flag、无环境开关、不 auto-create/seal；`validate` 只读、`assess` 只写显式 `--output`）+ orchestration 级对抗 fault campaign（ladder 中段崩溃两臂重放 finalize、跌停/停牌日保留 mandate 补卖不超卖、长期 pending 保留 ITT 行、lease takeover 阻断 entry 写 exit 存活、divergent 重 commit 永久 latch）+ AST import 边界守卫（trial path 永不 import broker/outbox/`shadow_trust`/`CapitalAuthorizationEnvelope`/`ExecutionPermit`/authority 写面/激活方法/legacy court-backtest，`gateway.exits` 是唯一允许的 gateway 子模块）。权威文档：spec §12、migration `2026-08-10-policy-v2-shadow-decision-v3.md`、runbook `v3-btst-regime-forward-trial.md`。基线 `runtime_mode "off"` + 全 0 caps + `execution_authority "NONE"` 三重 fail-closed；无 broker 连接、无 authority flip、无真实资本激活。
