...
4. **`known_distributions.py` 是硬编码常量**（现行生效 BTST T+10 为 2026-07-12 校准的 626 票样本、连续涨停口径、未扣费，早于 07-18 锚定修正与 08-16 执行重建，未按两者重验），无自动刷新，引用前需交叉验证。2026-08-18 起展示层对齐：`Distribution.provenance` 字段披露样本出处，`--daily-action` 先验行不再自称「全池回测」（改「历史回测 · 未扣费」+ 区末一次性脚注含执行口径参考 +3.4%/57%）；文件头受污染的 +8.15% 佐证已移除（保留为标注过的审计线索）。**改常量数值 = 策略行为变化，需新证据世代；provenance 只做披露不进 Kelly。**
