...
**目标架构仍未上线，也没有资本授权。** 仍未实现 activation CAS、Growth Kernel、Capital Gateway、entry/exit permit、broker connection、authority flip、签名服务或任何可执行资本路径；Plan 03 的 Authorizer/Issuer 只产生 INACTIVE 候选 envelope。现有 Revision 1 ports/`DecisionSeal` 只能留在显式 compatibility namespace，不得进入 final `CapabilityVerifier` 或继续作为最终接口扩散。
