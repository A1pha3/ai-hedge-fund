...
class CapitalAuthorizationEnvelope(CanonicalModel):
    ...
    def validate_envelope(self) -> Self:
        ...
        if len(budget_programs) != len(set(budget_programs)) or set(
            budget_programs
        ) != set(self.research_program_ids):
            raise ValueError("program loss budgets must exactly bind research programs")
        if {grant.research_program_id for grant in self.lineage_grants} - set(
            self.research_program_ids
        ):
        ...
        exploration_cap = sum(
            (grant.lineage_gross_cap for grant in exploration), start=0
        )
