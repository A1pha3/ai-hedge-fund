...
The current ``ShadowDecision`` is schema major 4. Historical schema-major-2
and schema-major-3 artifacts remain audit-readable through frozen legacy
shapes. There is deliberately no upgrader: nothing rewrites old bytes into
the new schema, and the official paired-trial runner rejects legacy artifacts.
"""
...
from src.screening.offensive.v3.contracts.trial import ShadowPolicyBinding
...
class LegacyShadowOrderLineV3(CanonicalModel):
    """The schema-major-3 shadow line shape, frozen for audit."""
...
class LegacyShadowDecisionV3(CanonicalModel):
    """The schema-major-3 policy-bound shadow artifact, read-only."""
    ...
    evidence_set_merkle_root: Sha256
    shadow_stage_binding: ShadowStageBinding
    counterfactual_lines: Annotated[
        tuple[LegacyShadowOrderLineV3, ...], Field(min_length=1)
    ]
    cost_assumption_version: str
    execution_assumption_version: str
    created_at: UtcInstant
    ...
    def validate_shadow(self) -> Self:
        ...
        if self.available_at < self.created_at:
            raise ValueError("shadow available_at cannot precede created_at")
        line_ids = [line.shadow_line_id for line in self.counterfactual_lines]
        if len(line_ids) != len(set(line_ids)) or line_ids != sorted(line_ids):
            raise ValueError("shadow line IDs must be unique and canonical")
        for line in self.counterfactual_lines:
            if (
                line.producer_namespace,
                line.family_id,
...
LegacyShadowDecision: TypeAlias = LegacyShadowDecisionV2 | LegacyShadowDecisionV3


def read_shadow_decision_json(payload: bytes, *, official_trial: bool) -> LegacyShadowDecision:
    """Decode one historical schema-major-2/3 shadow decision, read-only.
    ...
    if header.get("schema_major") == 2:
        return LegacyShadowDecisionV2.model_validate_json(payload, strict=True)
    if header.get("schema_major") == 3:
        return LegacyShadowDecisionV3.model_validate_json(payload, strict=True)
    raise ShadowCompatibilityError(
        "unsupported_legacy_shadow_schema",
        "only historical shadow schema majors 2 and 3 are compatibility-readable",
    )
