...
def compute_stock_detail(
    ...
    if consecutive_map is None:
        try:
            ...
            # NS-17 / BH-017 family (c324, loop 75): best-effort fallback 到 0 是
            # 有意为之 (不阻塞渲染), 但之前完全静默 — 失败与"真实无连续推荐"
            # 在 render 上不可区分 (line 532: 连续推荐: 0天)。tracking_history.json
            # 损坏 / schema drift 会静默清零所有标的的连续推荐显示。surface 到
            # logger.warning 让 operator 能把"连续推荐归零"与 compute 失败关联。
            logger.warning(
                "compute_consecutive_recommendations 失败, 连续推荐显示退化到 0 天: %s",
                e,
                exc_info=True,
            )
            consecutive_days = 0
    ...
    if decay_map is None:
        try:
            ...
            # NS-17 / BH-017 family (c324, loop 75): 同上 — fallback 到 "none" 是
            # 有意, 但必须 surface; 否则 report schema drift 时所有标的信号衰减
            # 静默归 none, 与"真实无衰减"不可区分。
            logger.warning(
                "detect_signal_decay 失败, 信号衰减显示退化到 none: %s",
                e,
                exc_info=True,
            )
            decay_level = "none"
