...
- **bootstrap CI**: 每个 winrate 点估计附带 percentile bootstrap 置信区间
  (默认 2000 重采样, 95%, seed=42), 本模块 ``_winrate_bootstrap_ci`` 自实现
...
def _winrate_bootstrap_ci(
    ...
    *,
    ...
    """Bootstrap percentile CI on winrate point estimate.

    对 returns 做有放回重采样 (保持样本量 = len(returns)), 每次算 winrate,
    取 (alpha/2, 1-alpha/2) 百分位区间. 幂等: 同 seed+同 input → 同 output.

    Returns:
        (ci_lower, ci_upper): winrate 置信区间; 都 None 仅当 returns 为空.
    """
    n = len(returns)
    if n == 0:
        return None, None
    ...
    return boot_winrates[lower_idx], boot_winrates[upper_idx]
...
def compute_regime_historical_winrates_from_records(
    ...
    *,
    ...
    Args:
        ...
        bootstrap_seed: bootstrap PRNG seed (默认 42, 幂等).
    ...
    for regime, recs in regime_records.items():
        ...
        regime_winrates[regime] = _compute_stats(
            t30_returns,
            n_bootstrap=n_bootstrap,
            ci_level=ci_level,
            seed=bootstrap_seed,
        ...
        for horizon in horizons:
            ...
            if len(horizon_returns) < min_samples:
                ...
                # 只含成熟 horizon 的语义一致)
                continue
            multi[horizon] = _compute_multihorizon_stats(
                horizon_returns,
                n_bootstrap=n_bootstrap,
                ci_level=ci_level,
                seed=bootstrap_seed,
