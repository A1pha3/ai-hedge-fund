...
def _attach_primary_focus_contract(
    ...
    for item in focus_items:
        if item.get("focus_tier") == "primary_entry" and str(item.get("ticker") or "").strip() == primary_ticker:
...
def _sort_opening_focus_items(focus_items: list[dict[str, Any]]) -> None:
    focus_items.sort(
        key=lambda item: (
            0 if item.get("focus_tier") == "primary_entry" else 1 if item.get("focus_tier") == "near_miss_watch" else 2 if item.get("focus_tier") == "opportunity_pool" else 3 if item.get("focus_tier") == "no_history_observer" else 4,
