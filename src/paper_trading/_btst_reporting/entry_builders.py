...
def _resolve_selected_execution_quality_thresholds() -> dict[str, Any]:
    ...
    return {
        "selected_zero_follow_through_min_evaluable_count": int(thresholds.get("selected_zero_follow_through_min_evaluable_count") if thresholds.get("selected_zero_follow_through_min_evaluable_count") is not None else WEAK_NEAR_MISS_DEMOTION_MIN_EVALUABLE_COUNT),
        "selected_intraday_only_min_evaluable_count": int(thresholds.get("selected_intraday_only_min_evaluable_count") if thresholds.get("selected_intraday_only_min_evaluable_count") is not None else WEAK_NEAR_MISS_DEMOTION_MIN_EVALUABLE_COUNT),
        "selected_intraday_only_max_next_close_positive_rate": float(thresholds.get("selected_intraday_only_max_next_close_positive_rate") if thresholds.get("selected_intraday_only_max_next_close_positive_rate") is not None else 0.0),
    }
...
def _collect_formal_execution_block_flags(selection_entry: dict[str, Any], short_trade_entry: dict[str, Any] | None = None) -> list[str]:
    ...
    return [flag for flag in FORMAL_EXECUTION_BLOCK_FLAGS if bool(selection_entry.get(flag)) or bool(short_trade_entry.get(flag))]
...
def _filter_execution_ready_entries(
    ...
    return [dict(entry) for entry in entries if not _is_formal_execution_blocked_entry(entry)]


def _resolve_formal_shadow_decision(selection_entry: dict[str, Any], short_trade_entry: dict[str, Any]) -> tuple[str, list[str]]:
    execution_blocked_flags = _collect_formal_execution_block_flags(selection_entry, short_trade_entry)
...
def _extract_upstream_shadow_entry(selection_entry: dict[str, Any], supplemental_entry: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    candidate_pool_lane = str(supplemental_entry.get("candidate_pool_lane") or replay_context.get("candidate_pool_lane") or _source_lane_label(candidate_source))
    candidate_pool_rank = supplemental_entry.get("candidate_pool_rank") or replay_context.get("candidate_pool_rank")
    decision, execution_blocked_flags = _resolve_formal_shadow_decision(selection_entry, short_trade_entry)
    ...
    return {
        ...
        "candidate_pool_avg_amount_share_of_cutoff": _round_or_none(supplemental_entry.get("candidate_pool_avg_amount_share_of_cutoff") or replay_context.get("candidate_pool_avg_amount_share_of_cutoff")),
        "candidate_pool_avg_amount_share_of_min_gate": _round_or_none(supplemental_entry.get("candidate_pool_avg_amount_share_of_min_gate") or replay_context.get("candidate_pool_avg_amount_share_of_min_gate")),
        "upstream_candidate_source": str(supplemental_entry.get("upstream_candidate_source") or replay_context.get("upstream_candidate_source") or "candidate_pool_truncated_after_filters"),
...
def _build_upstream_shadow_summary(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        decision = str(entry.get("reporting_decision") or entry.get("decision") or "rejected")
    ...
    return {
        ...
        "promotable_count": sum(1 for entry in entries if str(entry.get("reporting_decision") or entry.get("decision") or "") in {"selected", "near_miss"}),
...
def _reclassify_selected_execution_quality_entries(
    ...
    for entry in selected_entries:
        ...
        if execution_quality_label == "zero_follow_through" and evaluable_count >= int(demotion_thresholds["selected_zero_follow_through_min_evaluable_count"]):
        ...
        if execution_quality_label == "intraday_only" and evaluable_count >= int(demotion_thresholds["selected_intraday_only_min_evaluable_count"]) and next_close_positive_rate <= float(demotion_thresholds["selected_intraday_only_max_next_close_positive_rate"]):
            demoted_entry = dict(updated_entry)
            demoted_entry["demoted_from_decision"] = "selected"
            demoted_entry["reporting_bucket"] = "selected_execution_demoted"
