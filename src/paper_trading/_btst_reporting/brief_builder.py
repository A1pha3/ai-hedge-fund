...
def _collect_formal_execution_block_flags(selection_entry: dict[str, Any], short_trade_entry: dict[str, Any] | None = None) -> list[str]:
...
def _build_btst_brief_recommendation_lines(
    *,
    ...
    if primary_entry is None:
        blocked_line = _build_execution_blocked_brief_recommendation_line(selection_targets)
        if blocked_line:
            recommendation_lines.insert(0, blocked_line)
    runner_recall_line = _build_runner_recall_review_recommendation_line(runner_recall_review_entries)
...
def _build_legacy_btst_next_day_trade_brief_payload(
    *,
    ...
    brief_frontier_context = {
        "catalyst_theme_frontier_summary": catalyst_theme_frontier_summary,
        "catalyst_theme_frontier_priority": _build_catalyst_theme_frontier_priority(catalyst_theme_frontier_summary, []),
        "upstream_shadow_entries": upstream_shadow_entries,
        "upstream_shadow_summary": _build_upstream_shadow_summary(upstream_shadow_entries),
    }
    return _build_btst_next_day_trade_brief_payload(
...
def _load_btst_brief_inputs(input_path: str | Path, trade_date: str | None) -> dict[str, Any]:
    try:
        ...
        if not resolved_input.is_dir() or "selection_artifacts directory not found under" not in str(exc):
            raise
        ...
        replay_input = {}
        legacy_mode = True
        legacy_next_trade_date = _normalize_trade_date(legacy_brief.get("next_trade_date"))
    ...
    return {
        ...
        "actual_trade_date": _normalize_trade_date(snapshot.get("trade_date") or legacy_brief.get("trade_date") or trade_date),
...
def _build_btst_brief_candidate_groups(*, snapshot: dict[str, Any], selection_targets: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    return {
        "selected_entries": [entry for entry in short_trade_entries if entry.get("reporting_decision") == "selected"],
        "near_miss_entries": [entry for entry in short_trade_entries if entry.get("reporting_decision") == "near_miss"],
...
def _build_btst_brief_short_trade_entries(
    ...
    short_trade_entries = [candidate for candidate in (_extract_short_trade_entry(entry) for entry in selection_targets.values()) if candidate is not None]
    short_trade_entries.sort(
        key=lambda entry: (
            0 if entry.get("reporting_decision") == "selected" else 1 if entry.get("reporting_decision") == "near_miss" else 2,
...
def _build_btst_brief_opportunity_pool_entries(
    ...
    opportunity_pool_entries = [candidate for candidate in (_extract_short_trade_opportunity_entry(entry) for entry in selection_targets.values()) if candidate is not None]
...
def _build_btst_brief_research_upside_radar_entries(
    ...
    research_upside_radar_entries = [candidate for candidate in (_extract_research_upside_radar_entry(entry) for entry in selection_targets.values()) if candidate is not None]
...
def _build_btst_brief_catalyst_theme_entries(
    ...
    catalyst_theme_entries = [candidate for candidate in (_extract_catalyst_theme_entry(entry) for entry in (snapshot.get("catalyst_theme_candidates") or [])) if candidate is not None]
...
def _build_btst_brief_catalyst_theme_shadow_entries(
    ...
    catalyst_theme_shadow_entries = [candidate for candidate in (_extract_catalyst_theme_shadow_entry(entry) for entry in (snapshot.get("catalyst_theme_shadow_candidates") or [])) if candidate is not None]
...
def _build_supplemental_short_trade_entry_map(
    ...
    return {str(entry.get("ticker") or ""): dict(entry) for entry in list(replay_input.get("supplemental_short_trade_entries") or []) if entry.get("ticker")}
    ...
    return {
        ...
        for candidate in (
            _extract_upstream_shadow_entry(
                entry,
                supplemental_short_trade_entry_by_ticker.get(str(entry.get("ticker") or "")),
            )
...
def _merge_replay_only_upstream_shadow_entries(
    ...
    for candidate in (_extract_upstream_shadow_replay_only_entry(entry) for entry in list(replay_input.get("upstream_shadow_observation_entries") or []) if entry.get("ticker")):
...
def _build_excluded_research_entries(
    ...
    excluded_research_entries = [candidate for candidate in (_extract_excluded_research_entry(entry) for entry in selection_targets.values()) if candidate is not None]
...
def _build_btst_brief_summary(
    *,
    ...
    execution_blocked_summary = _build_execution_blocked_candidate_summary(selection_targets)
    return {
        ...
        "short_trade_formal_block_flag_counts": dict(reporting_target_summary.get("short_trade_formal_block_flag_counts") or {}),
        "short_trade_formal_non_halt_blocked_selected_count": _summary_value(
            reporting_target_summary,
            "short_trade_formal_non_halt_blocked_selected_count",
            0,
        ),
        "short_trade_formal_non_halt_gate_counts": dict(reporting_target_summary.get("short_trade_formal_non_halt_gate_counts") or {}),
        "short_trade_formal_non_halt_prior_quality_counts": dict(reporting_target_summary.get("short_trade_formal_non_halt_prior_quality_counts") or {}),
...
def _build_runner_recall_review_entry(entry: dict[str, Any], bucket_name: str) -> dict[str, Any] | None:
    ...
    if str(historical_prior.get("execution_quality_label") or "") in RUNNER_RECALL_REVIEW_EXCLUDED_EXECUTION_QUALITY_LABELS:
        return None
    if _as_float(entry.get("research_score_target")) <= 0 and _as_float(entry.get("score_target")) < OPPORTUNITY_POOL_MIN_SCORE_TARGET:
        return None
    return {
        **dict(entry),
        "runner_recall_bucket": bucket_name,
        "runner_recall_note": ("来源于 watchlist_filter_diagnostics 的 payoff-first runner recall 复审层；" "只做影子复审，不直接加入 formal BTST 执行名单。"),
    }
...
def _build_runner_recall_review_recommendation_line(
    ...
    if not runner_recall_review_entries:
        return None
    preview = ", ".join(str(entry.get("ticker") or "") for entry in runner_recall_review_entries[:3] if entry.get("ticker"))
    suffix = " 等" if len(runner_recall_review_entries) > 3 else ""
    return f"payoff-first runner recall 复审层优先回看 {preview}{suffix}；" "这些票来自 watchlist_filter_diagnostics，只做影子复审，不直接并入 formal BTST 执行名单。"
...
def _build_btst_brief_decision_counts(
    ...
    return {
        "blocked_count": sum(1 for entry in selection_targets.values() if _is_formal_execution_blocked_target(entry)),
        "rejected_count": sum(1 for entry in selection_targets.values() if (entry.get("short_trade") or {}).get("decision") == "rejected"),
        "research_selected_count": sum(1 for entry in selection_targets.values() if (entry.get("research") or {}).get("decision") == "selected"),
    }


def _is_formal_execution_blocked_target(selection_entry: dict[str, Any], short_trade_entry: dict[str, Any] | None = None) -> bool:
