...
def append_premarket_overview_markdown(lines: list[str], card: dict[str, Any]) -> None:
    ...
    lines.append(f"- runner_recall_review_count: {summary.get('runner_recall_review_count')}")
    ...
    if execution_contract:
        lines.append(f"- report_mode: {execution_contract.get('report_mode') or 'n/a'}")
        lines.append(f"- effective_trade_bias: {execution_contract.get('effective_trade_bias') or 'n/a'}")
        if execution_contract.get("execution_state") not in (None, ""):
            lines.append(f"- execution_state: {execution_contract.get('execution_state')}")
        if execution_contract.get("max_allowed_state_today") not in (None, ""):
            lines.append(f"- max_allowed_state_today: {execution_contract.get('max_allowed_state_today')}")
        lines.append(f"- release_authority: {execution_contract.get('release_authority') or 'none'}")
    lines.append("")


def append_premarket_action_block(lines: list[str], entry: dict[str, Any], *, indexed: int | None = None) -> None:
    ...
    if entry.get("max_allowed_state_today") not in (None, ""):
        lines.append(f"{prefix}max_allowed_state_today: {entry.get('max_allowed_state_today')}")
    if entry.get("release_authority") not in (None, ""):
        lines.append(f"{prefix}release_authority: {entry.get('release_authority')}")
    ...
    payoff_note = _format_historical_payoff_note(dict(entry.get("historical_prior") or {}))
    if payoff_note:
...
def append_premarket_action_section(lines: list[str], title: str, entries: list[dict[str, Any]]) -> None:
    _append_titled_indexed_section(
        lines,
        title=f"## {title}",
        items=entries,
        render_item=lambda inner_lines, entry, index: append_premarket_action_block(inner_lines, entry, indexed=index),
    )


def append_candidate_watch_scoring_fields(lines: list[str], item: dict[str, Any]) -> None:
...
def append_candidate_watch_reason_tags(lines: list[str], item: dict[str, Any], *, reasons_label: str) -> None:
...
def append_premarket_excluded_entries_markdown(lines: list[str], excluded_entries: list[dict[str, Any]]) -> None:
    lines.append("## Explicit Non-Trades")
    if not excluded_entries:
        _append_none_block(lines)
        return
    lines.extend(f"- {entry.get('ticker')}: research selected, but short_trade={entry.get('short_trade_decision')} so it stays outside the short-trade execution list." for entry in excluded_entries)
    lines.append("")


def append_premarket_primary_action_markdown(lines: list[str], primary_action: Any) -> None:
...
def append_premarket_rollout_validation_markdown(lines: list[str], rollout_validation: dict[str, Any]) -> None:
    ...
    lines.append(f"- selected_hit_rate_15pct: {_format_rollout_value(rollout_validation.get('selected_hit_rate_15pct'), 4)} -> {_format_rollout_value(rollout_validation.get('shadow_hit_rate_15pct'), 4)}")
