        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    # NS-17/BH-017 同族 (c278): bold 变体注册失败静默 pass
                    # 会让字体退化 (bold 退化为 regular) 不可观测. debug 级别
                    # (冷路径, 每次 PDF 导出 1 次, 纯展示层非决策链).
                    logger.debug(
                        "pdf_exporter: CJK bold variant register failed (font=%s, path=%s): %s",
                        name,
                        path,
                        exc,
                    )
            ...
            # NS-17/BH-017 同族 (c278): 候选字体整体注册失败静默 continue
            # 会让字体回退链不可观测. debug 级别 (冷路径, 纯展示层).
            logger.debug(
                "pdf_exporter: CJK font candidate register failed (path=%s, name=%s): %s",
                path,
                name,
                exc,
            )
