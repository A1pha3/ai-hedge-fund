...
async def get_prices(ticker: str, start_date: str, end_date: str, use_cache: bool = True, validate: bool = True) -> List[Price]:
    ...
    response = await router.get_prices(ticker=ticker, start_date=start_date, end_date=end_date, use_cache=use_cache)
...
async def get_financial_metrics(ticker: str, end_date: str, limit: int = 10, use_cache: bool = True, validate: bool = True) -> List[FinancialMetrics]:
    ...
    response = await router.get_financial_metrics(ticker=ticker, end_date=end_date, limit=limit, use_cache=use_cache)
...
async def get_company_news(ticker: str, start_date: str, end_date: str, use_cache: bool = True, validate: bool = True) -> List[CompanyNews]:
    ...
    response = await router.get_company_news(ticker=ticker, start_date=start_date, end_date=end_date, use_cache=use_cache)
...
def get_price_data_sync(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    """
    同步获取价格数据 DataFrame
