...
class TushareProvider(BaseDataProvider):
    ...
    def _init_tushare(self):
        ...
        try:
            ...
            # akshare/tushare 未安装是 dev box 的预期状态, 不需告警
            self.health_status = "unhealthy"
        except Exception as e:
            # NS-17 / BH-017 family sibling: 非 ImportError 失败 (token revoked,
    ...
    async def get_prices(
        ...
        try:
            ...
            try:
                ...
                # NS-17 / BH-017 family sibling: adj_factor 抓取失败时降级到未复权
                # 价格是有意为之 (backtest 仍可跑, 仅在 ex-div 日有 phantom loss),
                # 但之前完全静默 — 运维无法感知 backtest 结果被未复权价格污染。
                # surface 到 logger.warning 让 operators 能检测到此降级触发,
                # 并与下游 return/ATR/stop-loss/drawdown 异常关联定位。
                logger.warning(
