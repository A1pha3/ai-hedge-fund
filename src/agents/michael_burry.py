...
def _analyze_value(metrics, line_items, market_cap):
    ...
    # EV/EBITDA (from financial metrics)
    if metrics:
        ev_ebitda = getattr(metrics[0], "enterprise_value_to_ebitda_ratio", None)
        if ev_ebitda is not None:
            if ev_ebitda < 6:
                score += 2
                details.append(f"EV/EBITDA {ev_ebitda:.1f} (<6)")
            elif ev_ebitda < 10:
                score += 1
                details.append(f"EV/EBITDA {ev_ebitda:.1f} (<10)")
            else:
...
def _generate_burry_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                with_fact_grounding_rules("""You are an AI agent emulating Dr. Michael J. Burry. Your mandate:
