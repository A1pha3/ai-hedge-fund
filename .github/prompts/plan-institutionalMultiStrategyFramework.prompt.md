...
> **基于文档**：institutional_multi_strategy_framework_v1.4.md  
> **制定日期**：2026-03-06  
...
| 能力 | 对应代码 | 覆盖度 | 备注 |
|------|---------|--------|------|
| 技术指标计算（EMA/ADX/ATR/RSI/布林带/Hurst） | `src/agents/technicals.py` | ✅ 完整 | 含 5 个子策略（趋势/均值回归/动量/波动率/统计套利） |
| 基本面分析 | `src/agents/fundamentals.py` | ✅ 完整 | 盈利/增长/财务健康评分 |
| 估值分析 | `src/agents/valuation.py` | ✅ 完整 | 多模型估值 |
| 成长分析 | `src/agents/growth.py` | ✅ 完整 | 增速趋势评估 |
...
| **涨跌停处理器**（待买/待卖队列） | §4.4, §5.3 | **P0** | 退出管理器 |
| **行业暴露计算器**（申万一级分类） | §4.2 | **P1** | tushare 行业分类数据 |
...
| **回测增强**（Calmar/盈亏比/换手率/CVaR） | §8.2 | **P2** | 现有回测引擎扩展 |
...
├── agents/                       # 现有（需微调）
├── backtesting/                  # 现有（需扩展）
│   └── metrics.py                # 扩展：Calmar/盈亏比/换手率/CVaR
├── data/                         # 现有（需扩展）
│   └── providers/
...
│                                                        ▼               │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────────┐   │
│  │ 退出管理器    │────▶│ 仓位计算器    │◀───│ Score_final 聚合      │   │
│  │ exit_manager  │     │ pos_calculator│     │ 0.4×B + 0.6×C        │   │
│  └──────────────┘     └──────┬───────┘     └──────────────────────┘   │
│                              │                                         │
│                              ▼                                         │
│                    ┌──────────────────────┐                            │
...
class FusedScore:
    """Layer B 融合得分"""
    score_b: float           # -1 ~ +1
    strategy_signals: dict   # {strategy_name: StrategySignal}
    arbitration_applied: list # 触发的仲裁规则
    market_state: str        # trend/range/mixed
    weights_used: dict       # 实际使用的策略权重
...
class FinalScore:
    ...
    score_b: float
    score_c: float
    agent_signals: dict      # {agent_name: StrategySignal}
...
class HoldingState:
    ...
    holding_days: int
    profit_take_stage: int   # 0/1/2/3（止盈阶段）
...
| 0.1.6 | `src/monitoring/__init__.py` | 创建 monitoring 包 |
...
| 0.3.6 | `src/backtesting/types.py` | 扩展 PerformanceMetrics 含新指标字段 |
...
| 1.3.2 | 包含字段：ticker, name, industry_sw, market_cap, avg_volume_20d, disclosure_risk |
...
| 子任务 | 策略 | 复用来源 | 新增/适配 |
|--------|------|---------|-----------|
| 2.1.1 | 策略 A（趋势） | `technicals.py` → `calculate_trend_signals()` + `calculate_momentum_signals()` | 适配输出为 StrategySignal（方向/置信度/完整度），均线改为 EMA(10/30/60) |
| 2.1.2 | 策略 B（均值回归） | `technicals.py` → `calculate_mean_reversion_signals()` + `calculate_stat_arb_signals()` | 适配输出，增加 Hurst 判定逻辑 |
| 2.1.3 | 策略 C（基本面/估值） | `fundamentals.py` + `valuation.py` + `growth.py` | 提取评分逻辑为纯函数，不走 LLM；需新增行业 PE 中位数对比 |
...
4. 加权平均完整度：completeness = mean(comp_k)（剔除 comp=0 的子因子）
5. 置信度截断至 [0, 100]
```
...
| 规则 | 触发条件 | 动作 | 优先级 |
|------|---------|------|--------|
| 规则一：安全优先 | 任一策略强看跌(conf≥75) + 基本面方向=-1 | 强制"回避" + 进入冷却期 | 最高 |
| 规则二：时间框架分级 | 短周期 vs 长周期策略冲突 | 按绝对贡献度推断持仓期限，调整权重 | 高 |
...
| test_safety_first_rule | 强看跌+基本面负面 → 强制回避 |
...
| 3.3.6 | Level 5: 分批止盈 | +15% 卖 50%；+25% 再卖 30%；剩余移动止盈 |
...
| test_staged_profit_take | 三阶段止盈比例正确 |
...
class DailyPipeline:
    def run_post_market(self, trade_date: str) -> ExecutionPlan:
        ...
        # Step 3: Layer C 多智能体深度分析
        agent_results = self.run_agent_analysis(high_pool, trade_date)  # 现有 LangGraph 工作流
        final_scores = self.aggregate_layer_c(fused, agent_results)
        watchlist = [s for s in final_scores if s.score_final >= 0.25]  # ~10 只

        # Step 4: 生成执行计划
        plan = self.plan_generator.generate(watchlist, self.portfolio, trade_date)
        return plan
...
| 板块强度 | 所属行业排名前 50%（含尾盘异动容忍） | akshare 行业指数/日线涨跌 |
...
| 缩量市 | 连续 3 日成交额 < 4000 亿 | 逐日减仓至半仓 |
| 多票同时止损 | 系统性暴跌 | 按亏损→流动性→策略贡献排序卖出 |
| 回撤 -10% | 组合净值回撤 | 预警：暂停新开仓 |
| 回撤 -15% | 组合净值回撤 | 强制减仓 50% + 恢复协议（5日冷却 + 逐步恢复） |
...
| test_recovery_protocol | 回撤 -15% 后恢复路径正确 |
...
| 里程碑 | Phases | 交付物 | 交付标准 |
|--------|--------|--------|---------|
| **M0: 基础就绪** | Phase 0 | 数据接口通烟测试 + 回测新指标 + 目录骨架 | 所有新接口返回有效数据 |
...
| **M4: 全流水线贯通** | Phase 4 | 日度 7 步执行流水线 | 端到端运行无异常 |
...
| Layer C 1080 次 LLM 调用成本/延迟 | 日运行成本过高或超时 | 中 | 先用 GPT-4o-mini 初筛，Top 20 用精度模型 |
...
| 申万行业分类数据 | ⚠️ 需获取 | tushare `index_classify` 接口 |
