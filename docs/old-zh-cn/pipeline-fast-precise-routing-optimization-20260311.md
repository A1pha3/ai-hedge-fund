...
说明：这里记录的是 2026-03-11 当天实验时的环境变量快照。当前主链路已统一改为读取 `ARK_MODEL`，不再把 `ARK_FALLBACK_MODEL` 作为 provider route 主模型入口。
...
同样地，这一项仅代表当时的性能实验快照；当前仓库的现行 route 语义以 `ARK_MODEL` 为准。
