...
快照系统位于数据返回路径上，作为数据流的一个旁路分支。当数据从缓存或外部 API（Tushare、Financial Datasets）获取并转换为 Pydantic 模型后，在返回给 Agent 之前根据配置决定是否触发快照生成。无论数据来自缓存还是新鲜获取，快照系统都以相同方式工作。这种旁路设计确保了快照生成失败不会影响主流程的正常运行，符合失败隔离的原则。
...
文件分为五个主要分区。标题区、价格区、财务指标区、财务报表数据区、估值数据区。每个分区以二级标题分隔，便于快速导航。这种分区设计不是孤立的，而是反映了数据之间的内在联系。例如，估值数据区会引用财务指标区的数据，而财务指标区又与财务报表数据区存在勾稽关系。
...
财务报表数据区展示公司三大财务报表的核心数据，包括资产负债表（总资产、总负债、股东权益）、利润表（营业收入、净利润）和现金流量表（自由现金流、资本支出）。这部分数据来源于 LineItem 模型，字段是动态的，根据不同的数据源和公司可能包含不同的项目。
...
数值格式化采用金额单位。对于大额数值，自动转换为亿元或万元单位，并在表头标注单位。这种处理避免了表格中出现过长的大数，提高了可读性。例如，总资产 15000000000 元会显示为 150 亿元。
...
由于索引文件是单个文件，多个线程同时写入可能导致数据损坏。在 LangGraph 中多个 Agent 可能并行执行，导致多个 api.py 函数调用同时触发快照生成，因此索引文件的更新需要使用线程锁保护。实现时可使用 threading.Lock 确保同一进程内索引写入的串行化。

如果进一步扩展为多进程并发（如使用多进程进行批量分析），则需要引入跨进程的文件锁机制。可以使用 Python 的 fcntl 或 portalocker 库实现文件锁，确保跨进程的写入安全。
