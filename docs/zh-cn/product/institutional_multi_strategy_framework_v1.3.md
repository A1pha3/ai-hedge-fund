...
> **变更记录**：v1.2.1→v1.3 融合公式归一化约束、Layer C completeness 一致性、CVaR 方法指定、历史压力测试、多票止损排序、策略容量估算、候选池动态范围、参数速查补全
...
$$Score_{final} = \sum_{i} w_i \times direction_i \times confidence_i \times completeness_i \quad \text{s.t.} \quad \sum_{i} w_i = 1,\; w_i \geq 0$$
...
**执行顺序**：先由 3.3 节冲突仲裁规则调整各策略权重 → 再将调整后的权重归一化 → 最后代入上述融合公式计算组合得分。即仲裁在前、融合在后。
