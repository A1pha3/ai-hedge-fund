...
### 6.5 筛选结果增加股票名称 + 行业 (S) ✅ DONE 2026-06-06
- **痛点**: `--auto` 模式输出 Top-10 表格只显示 6 位代码 (如 `000001`), 用户需要手动查证每只票的名字和行业
- **现状**: `FusedScore` 模型只有 `ticker` 字段, `CandidateStock` 的 `name` / `industry_sw` 在 `fuse_batch()` 中被丢弃
- **改进方案**: 在 `FusedScore` 增加 `name`/`industry_sw` 字段, 在 `fuse_batch()` 传递, `_print_auto_screening_table()` 渲染 "000001 平安银行 银行"
- **预期收益**: Top-10 表格从"10 个匿名代码"变成"立即可读的股票推荐"
- **估算工作量**: S (0.5-1 天)
- **优先级**: P1

> 来源: 代码审查 `src/main.py:373-420`, `src/screening/models.py:91-103`。

### 6.6 选股推荐行业集中度预警 (S) ✅ DONE 2026-06-06
- **痛点**: `--auto` 输出 Top-10 中可能 6/10 属同一行业, 用户买入即承担集中风险, 但系统不提示
- **现状**: `src/portfolio/industry_exposure.py` 只计算组合持仓的行业集中度, 不覆盖选股推荐列表
- **改进方案**: 选出 Top N 后计算行业分布, 任一行业超 40% 打 `⚠️` 黄色警告, 并写入 JSON 报告
- **预期收益**: 防止最常见的选股失误 —— 买 5 只高度相关的票同时暴跌
- **估算工作量**: S (0.5-1 天)
- **优先级**: P1

> 来源: 代码审查 `src/main.py:run_auto_screening()`, 与 1.5 互补 (1.5 是持仓级, 本提案是推荐级)。

### 6.7 `--explain <ticker>` 评分解释快速查看 (S) ✅ DONE 2026-06-06
- **痛点**: 用户看到 `score_b=+0.3542` 但不知道为什么, 4 个策略信号的方向/置信度/子因子只在 JSON 文件中
- **改进方案**: 新增 `--explain <ticker>` CLI 标志, 读取最近的 `auto_screening_*.json`, 打印 10 行可读摘要:
  每个策略的方向箭头 + 置信度 + Top-3 子因子, 仲裁规则触发情况, 市场状态
- **预期收益**: "为什么推荐这只?" 从"打开 JSON 解析 4 个策略对象"变成一条命令, 直接提升决策质量
- **估算工作量**: S (1 天) — 新 `src/cli/explain.py` (~80 行) + `main.py` 参数解析
- **优先级**: P1

> 来源: 代码审查 `src/main.py`, `src/screening/signal_fusion.py`。

### 6.8 日间选股结果 Diff ("昨天 vs 今天变了什么") (M)
- **痛点**: 用户每天跑 `--auto`, 但无法回答"今天新进了哪些? 哪些掉了? 排名怎么变?"
- **现状**: 提案 2.2 的 `aggregate_screening_daily_digest.py` 做的是月度聚合统计, 不是逐票排名对比
- **改进方案**: (a) 新增 `scripts/diff_screening_results.py` 接受两个日期, 输出 `NEW Entrants` / `Dropped Out` / `Rank ↑↓ Movers`。 (b) 在 `--auto` 模式加 `--diff` 标志自动对比前一日报
- **预期收益**: "变了什么?" 是用户每次筛选后的第一个问题, 新进/排名变化是 30 天工作流中 alpha 最高的信号
- **估算工作量**: M (2-3 天)
- **优先级**: P1

> 来源: 代码审查 `src/main.py`, 与 2.2 互补 (2.2 是池统计, 本提案是逐票排名对比)。
...
- [x] 6.5 筛选结果增加股票名称+行业 (S) ✅ DONE 2026-06-06
- [x] 6.6 选股推荐行业集中度预警 (S) ✅ DONE 2026-06-06
- [x] 6.7 `--explain` 评分解释快速查看 (S) ✅ DONE 2026-06-06
- [ ] 6.8 日间选股结果 Diff (M) ✅ DONE 2026-06-06
