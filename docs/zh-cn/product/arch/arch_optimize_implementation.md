...
> 前置短线扩覆盖摘要：关于“Layer C 之前的短线候选该从哪条准入底线开始扩覆盖”的短版结论，见 [pre_layer_short_trade_boundary_coverage_expansion_summary_20260329.md](./pre_layer_short_trade_boundary_coverage_expansion_summary_20260329.md)。
...
- 已继续补齐 short trade 分数构造定点诊断：`selection_target_score_diagnostics_300394_300502.json` 现可直接输出 replay 后的 `weighted_positive_contributions`、`weighted_negative_contributions`、`total_positive_contribution`、`total_negative_contribution` 与 thresholds。真实样本显示，`300394` 仍有较强正贡献（`trend_acceleration=0.1325`、`close_strength=0.1319`、总正贡献 `0.4281`），但被 `layer_c_avoid_penalty=0.12`、`stale_trend_repair_penalty=0.0564`、`extension_without_room_penalty=0.036` 明显压制；`300502` 的总正贡献仅 `0.1919`，低于总负贡献 `0.2254`，且 `breakout_freshness=0`、`volume_expansion_quality=0`。这把后续优化进一步拆成两类：`300394` 更像惩罚项 / 分数构造问题，`300502` 更像候选入口与 breakout 语义本身不匹配的问题。
...
- 已继续补齐 split penalty 与候选入口聚焦诊断：`selection_target_penalty_split_variants.json` / `.md` 进一步把 `no_bearish_conflict_block` 拆成 `lower_avoid`、`lower_stale`、`lower_extension`、`lower_avoid_plus_stale`、`penalty_triplet_relief` 等真实变体；结果显示 `300394` 的单项最大收益来自 `layer_c_avoid_penalty`（`0.2133 -> 0.2733`），其次是 stale（`0.2415`）和 extension（`0.2313`），组合变体最高到 `0.3195`，与 softer penalty weights 的 `0.3207` 基本一致。与此同时，`selection_target_candidate_entry_focus.json` / `.md` 已证明 `300394`、`300502` 两个样本的 `candidate_source` 都是 `watchlist_filter_diagnostics`，原因码均为 `decision_avoid` 与 `score_final_below_watchlist_threshold`；这意味着 `300502` 的更高优先级已不再是 penalty 微调，而是审查 `watchlist_filter_diagnostics -> short trade replay candidate` 的候选入口语义是否过宽。
- 已继续补齐候选入口过滤变体验证：`selection_target_candidate_entry_filter_variants.json` / `.md` 新增了排除 `watchlist_filter_diagnostics` 中 `decision_avoid + score_final_below_watchlist_threshold` 边界样本的 replay 变体。真实 `2026-03-26` 样本表明，这条规则会同时移除 `300394` 与 `300502`，把它们从 `blocked` 直接变成 `none`。这说明入口过滤方向本身是有效的，但当前规则粒度过粗，不能直接作为生产默认逻辑，因为它会误伤 `300394` 这类仍有较强正贡献、只是在 penalty 上受压的样本；因此下一轮候选入口收紧应进一步叠加 breakout_freshness / volume_expansion_quality / catalyst_freshness 等正向结构条件，而不是只按 `avoid + 低分边界` 粗暴排除。
- 已继续补齐选择性弱结构候选入口过滤验证：`selection_target_candidate_entry_selective_filter_variants.json` / `.md` 新增了 `exclude_watchlist_avoid_weak_structure_entries` 与 `no_bearish_conflict_exclude_watchlist_avoid_weak_structure_entries` 两个 replay 变体，在 `watchlist_filter_diagnostics + decision_avoid + score_final_below_watchlist_threshold` 的基础上进一步要求 `breakout_freshness <= 0.05`、`volume_expansion_quality <= 0.05`、`catalyst_freshness <= 0.05`。真实 `2026-03-26` 样本表明，该规则只会移除 `300502`，而会保留 `300394`；在 baseline 下结果从 `rejected=7, blocked=2` 变成 `rejected=7, blocked=1, none=1`，在 `no_bearish_conflict_block` 下则表现为 `300394` 继续作为 penalty 主导样本进入 `rejected`，`300502` 直接 `none`。这说明候选入口收紧已经可以从“方向正确但过粗”推进到“具备样本区分力”的弱结构过滤原型，后续应优先把这类正向结构条件继续扩成更稳健的入口语义设计。
- 已继续补齐选择性弱结构候选入口的 W1 长窗口方法学校验：`scripts/replay_selection_target_calibration.py` 新增 candidate-entry metric grid 能力与 `--breakout-freshness-max-grid`、`--volume-expansion-quality-max-grid`、`--catalyst-freshness-max-grid` 三组 CLI 参数，并在 `data/reports/paper_trading_window_20260202_20260313_w1_frozen_replay_m2_7_dual_target_replay_input_validation_20260329/selection_target_candidate_entry_metric_grid_w1.json` / `.md` 上完成 54 行网格回放。首轮实验暴露旧 frozen replay 源会因缺失 `strategy_signals` 把弱结构指标退化成零，从而误过滤 27 个 blocked entry；随后已补上“仅当 short-trade data gate 通过时才允许基于指标的候选入口过滤生效”的保护，并新增回归测试。保护生效后，W1 网格所有 54 行都回到 `filtered={}`、`mismatches=0`、`replayed={'blocked': 50}`；增强后的 eligibility 统计进一步显示，整个窗口里共有 27 个 entry 命中了 `watchlist_filter_diagnostics + decision_avoid + score_final_below_watchlist_threshold` 这组入口预条件，但 27 个全部停在 `metric_data_fail_count`，没有任何一个进入弱结构阈值比较。换句话说，这轮结果应被解释为“方法学边界已澄清，当前 W1 样本暂无可供该弱结构规则命中的高保真 entry”，而不是把候选入口收紧方向本身否定掉。
- 已继续补齐 `300502` 的候选入口语义前沿：`scripts/replay_selection_target_calibration.py` 现已把 candidate-entry metric grid 扩展为可扫描 `--trend-acceleration-max-grid`、`--close-strength-max-grid`，并支持 `focus_tickers` / `preserve_tickers` 的最小分离行汇总。真实 `2026-03-26` 高保真样本上，新产物 `selection_target_candidate_entry_semantic_frontier_300502_refined.json` / `.md` 给出的最小 preserving row 为 `trend_acceleration <= 0.34` 且 `close_strength <= 0.69`：它会把 `300502` 从 replay 候选中直接过滤为 `blocked -> none`，同时保留 `300394` 不被过滤。对应的 filtered metrics 也直接说明两者差异：`300502` 只有 `trend_acceleration=0.3374`、`close_strength=0.6883`，而 `300394` 仍有 `trend_acceleration=0.7362`、`close_strength=0.942`。这说明 `300502` 的问题不只是 breakout/volume/catalyst 为零，还可以被量化为“短线趋势确认和收盘强度都不达标”的候选入口语义样本；下一轮入口规则可以继续沿这条正向结构分离面外推，而不必回到 penalty 路线。
- 已继续补齐 `300502` 的候选入口子集前沿：candidate-entry metric grid 现已支持在 CLI 中使用 `none` 省略单个 metric 维度，从而直接搜索“最小子集规则”，而不是把 breakout、trend、volume、close、catalyst 拆成多轮人工实验。真实 `2026-03-26` 样本上的新产物 `selection_target_candidate_entry_subset_frontier_300502.json` / `.md` 显示，真正的最小 preserving row 甚至比 `trend_acceleration + close_strength` 联动更简单：仅要求 `volume_expansion_quality <= 0.0`，就已经足以把 `300502` 从 replay 候选中过滤为 `blocked -> none`，同时保留 `300394` 不被过滤。这个结果把 `300502` 的入口语义进一步压缩成一个更可操作的核心锚点：它首先是一个 zero-volume-expansion 样本，后续若需要做更稳健的生产规则，应优先以这个锚点为核心，再视需要叠加 breakout、trend 或 close 条件。
...
- 已继续把“扩覆盖”从方向判断推进成可验证的准入底线变体实验与过滤明细观测：新增 `scripts/analyze_short_trade_boundary_coverage_variants.py`、`scripts/analyze_short_trade_boundary_filtered_candidates.py` 及对应回归测试。真实 `2026-03-23` 到 `2026-03-26` 旧 `layer_b_boundary` 候选池上的 baseline 回放显示，当前 23 个样本在默认 short-trade boundary 准入底线下 `0` 个可放行，其中 `19` 个先死于 `breakout_freshness`、`3` 个死于 `catalyst_freshness`、`1` 个死于 `volume_expansion_quality`，说明大部分旧池样本并不是“轻微差一点”。过滤明细进一步显示，最接近放行的 edge candidate 主要集中在 `300308`、`300274`、`300502` 这类只差 `catalyst_freshness=0.12` 的样本。围绕这簇样本做定向变体后，若仅把 `catalyst_freshness_min` 从 `0.12` 放到 `0.00`，其余准入底线保持不变，可新增 `3` 个样本，且 `next_high_return_mean=0.0222`、`next_close_return_mean=0.0028`、`next_high_hit_rate@2%=0.6667`、`next_close_positive_rate=0.6667`；若再把 `volume_expansion_quality_min` 一并降到 `0.06`，样本虽增到 `4` 个，但 `next_close_return_mean` 已回落到 `-0.0036`、`next_close_positive_rate` 降到 `0.5`。这把下一轮前置主线进一步收紧为：如要扩覆盖，优先做 `catalyst_freshness_min=0.00` 的受控实验，不要先联动放松 volume floor；breakout floor 暂时也不应被当作“轻微扩覆盖”的主杠杆，因为当前 `19/23` 的 breakout 失败更像结构性缺失而不是边缘样本。
- 已继续把这条仅催化项扩覆盖建议推进到真实窗口候选生成路径，而不是停留在静态回放：新增 `scripts/analyze_short_trade_boundary_expansion_cases.py` 与 `scripts/run_short_trade_boundary_variant_validation.py` 及对应回归测试，并启动 `DAILY_PIPELINE_SHORT_TRADE_BOUNDARY_CATALYST_MIN=0.0` 的真实窗口 paper trading 变体。现在完整 `2026-03-23` 到 `2026-03-26` 四天窗口已经全部收口：整窗阻断报告显示 short-trade target 总数为 `33`，其中 `24` 个来自 `short_trade_boundary`，`near_miss=6`、`rejected_short_trade_boundary_score_fail=18`，且旧 `layer_b_boundary` score-fail 簇依旧保持为 `0`；整窗 pre-Layer C outcome 报告则显示这 `24` 个候选的 `next_high_return_mean=0.0471`、`next_close_return_mean=0.0186`、`next_high_hit_rate@2%=0.75`、`next_close_positive_rate=0.7083`，仍明显强于旧 `layer_b_boundary` 池。同时，新增的逐票案例报告已确认静态定向扩覆盖推荐变体释放的 `300308`、`300274`、`300502` 都属于纯 `catalyst_freshness` 单条门槛放行；而新补的 `scripts/analyze_short_trade_boundary_score_failures.py` 与 `scripts/analyze_short_trade_boundary_score_failures_frontier.py` 则进一步证明，这批 `18` 个 `short_trade_boundary` 分数失利样本并不是新的准入垃圾簇，而是 breakout/trend/volume/close 结构已过线、但大多仍距 `near_miss` `0.06+` 的分数前沿样本。前沿扫描显示 `18/18` 都存在 near-miss 挽救行，但仅 `1/18`（`2026-03-26 / 300383`）属于纯阈值低成本释放，其余都需要 stale/extension penalty 联动下调。这把下一轮前置主线再次收紧为：默认保留 `catalyst_freshness_min=0.00` 作为已通过完整窗口验证的准入变体，并把后续优化重心切到 `short_trade_boundary` 分数前沿，而不是继续寻找第二条准入底线。
- 已继续把 `short_trade_boundary` 分数前沿从“窗口级建议”推进成可执行的个案与复现股票两层实验入口：新增 `scripts/analyze_targeted_short_trade_boundary_release.py` 与 `scripts/analyze_short_trade_boundary_recurring_frontier_cases.py` 及对应回归测试。真实 `300383` 定向释放实验已确认，只把 `near_miss_threshold` 从 `0.46 -> 0.42` 就能让 `2026-03-26 / 300383` 从 `rejected -> near_miss`，且其余 `17` 个 rejected `short_trade_boundary` 样本完全不变；这说明它已经具备低污染的个案释放语义。与此同时，复现前沿队列也把重复样本收敛为 `600821` 与 `002015` 两只，其中 `600821` 在 `2026-03-23, 2026-03-25, 2026-03-26` 三天重复出现，最小挽救成本为 `0.10`，`002015` 则在 `2026-03-23, 2026-03-24, 2026-03-25` 三天重复出现，最小挽救成本为 `0.12`；两者都不属于纯阈值释放，而是需要 stale/extension penalty 联动下调。随后又把 `scripts/analyze_pre_layer_short_trade_outcomes.py` 扩展为支持 `--tickers` 定向过滤，并新增 `scripts/analyze_targeted_short_trade_boundary_release_outcomes.py`、`scripts/analyze_short_trade_boundary_recurring_frontier_dossiers.py`、`scripts/analyze_short_trade_boundary_frontier_ticker_dossier.py`、`scripts/analyze_short_trade_boundary_frontier_pair_comparison.py`、`scripts/analyze_recurring_frontier_ticker_release.py`、`scripts/analyze_recurring_frontier_ticker_release_outcomes.py`、`scripts/analyze_recurring_frontier_release_pair_comparison.py`、`scripts/analyze_short_trade_ticker_role_history.py`、`scripts/analyze_recurring_frontier_transition_candidates.py`、`scripts/analyze_multi_window_short_trade_role_candidates.py`、`scripts/analyze_multi_window_short_trade_ticker_dossier.py` 与 `scripts/analyze_multi_window_short_trade_ticker_pair_comparison.py`，把释放/前沿结果与真实次日表现合并成固定案卷、释放基线、跨窗口角色迁移证据，以及“局部基线 vs 稳定规则”的正式分类器。最新一轮又补上了自动发现历史窗口的能力，可直接从 `data/reports` 根目录递归发现带 `selection_snapshot.json` 的 `paper_trading_window_*` 报告；基于这项能力生成的 `recurring_frontier_transition_candidates_all_windows_20260329.md` 已覆盖 `14` 个自动发现窗口，且结论保持不变。随后新增的全局扫描 `multi_window_short_trade_role_candidates_20260329.{json,md}` 又把边界进一步明确：自动发现窗口范围内共有 `39` 个曾进入 short-trade 角色的 ticker，但只有 `4` 个 ticker 满足“至少 2 个 short-trade trade_date”，且 `stable_candidate_count = 0`；这 4 个 ticker 分别是 `600821`、`002015` 以及当前窗口内新出现的 near-miss 型样本 `001309`、`300620`，四者全部都仍属于 `emergent_local_baseline`。真实窗口准入窗口补做 outcome 对照后，`300383` 单票样本的 `next_open_return=0.0246`、`next_high_return=0.0527`、`next_close_return=0.0146`，说明它不仅是低污染释放，也是正向次日表现样本；`600821` 与 `002015` 的复现案卷则进一步拆出：`600821` 三次样本 `next_high_return_mean=0.0503`、`next_close_positive_rate=0.3333`，已可固定为复现前沿的盘中主样本；`002015` 三次样本 `next_high_return_mean=0.0339`、`next_close_positive_rate=0.6667`，更适合作为收盘延续对照样本。更进一步的 ticker-level 复现释放实验已确认，两只票都能在各自 recurring row 下实现 `3/3 rejected -> near_miss` 且 `changed_non_target_case_count=0`；释放结果成对对照则把角色彻底固定为：`600821` 是复现释放的盘中主样本，`002015` 是收盘延续对照样本。对 near-miss 新兴局部基线的新补案卷与成对对照则进一步说明，`001309` 的两次可用样本给出 `next_high_return_mean=0.0510`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0`，应作为 near-miss 收盘延续优先样本；`300620` 的两次可用样本给出 `next_high_return_mean=0.0479`、`next_close_return_mean=-0.0014`、`next_close_positive_rate=0.5`，更适合作为 near-miss 盘中对照样本。最后，新补的 ticker role history 报告、transition scanner、全局 multi-window 扫描器与 near-miss 案卷/成对对照又把边界进一步收紧：在更早的 `paper_trading_window_20260316_20260323_live_m2_7_20260323` 里，两只前沿股票都还只是旧 Layer B 池里的 `below_fast_score_threshold` 样本，只是在当前仅催化项窗口里才首次连续变成 recurring `short_trade_boundary_rejected`；而 `001309`、`300620` 也只是当前窗口的 `short_trade_boundary_near_miss` 型新兴局部基线，并没有跨窗口复现证据。因此它们现在都应被定义为“当前窗口内成立的局部基线”，而不是已经跨窗口稳定的历史规则。最新一轮新增的 `scripts/analyze_short_trade_release_priority_scoreboard.py` 与 `scripts/analyze_case_based_short_trade_entry_pair_comparison.py` 则把此前分散的个案与复现入口放到同一标尺上重新排序：`short_trade_release_priority_scoreboard_20260329.{json,md}` 给出的统一结论是 `001309` 第一、`300383` 第二、`002015` 第三、`600821` 第四；对应的 `case_based_short_trade_entry_pair_comparison_001309_vs_300383_20260329.{json,md}` 又进一步确认，`001309` 在 `adjustment_cost=0.02`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0` 这组指标上已经压过 `300383` 的单样本纯阈值释放。于是更准确的下一轮优先级应改写为：先做 `001309` 的 near-miss 晋级跟踪推进，再做 `300383` 的纯阈值释放，而 `002015`、`600821` 则继续保留为复现前沿的局部基线对照。
- 已继续把 near-miss 新兴局部基线从“观察样本”推进成受控晋级入口：新增 `scripts/analyze_targeted_short_trade_near_miss_release.py` 与对应回归测试，可对指定 `trade_date:ticker` 的 `short_trade_boundary` near-miss 样本执行定向 selected 释放审查。真实窗口报告 `targeted_short_trade_near_miss_release_001309_20260329.{json,md}` 已确认，`001309` 在 `2026-03-24` 与 `2026-03-25` 两个 near-miss 样本上，只需把 `select_threshold` 从 `0.58 -> 0.56` 就能实现 `2/2 near_miss -> selected`，且 `changed_non_target_case_count=0`；对照报告 `targeted_short_trade_near_miss_release_300620_20260329.{json,md}` 则显示，`300620` 若要实现同样的 `2/2 near_miss -> selected`，需要把 `select_threshold` 进一步放到 `0.53`。这把 near-miss 旁路的内部优先级再次收紧为：`001309` 已可视为低污染、纯阈值的个案晋级样本，而 `300620` 仍更适合作为需要更激进阈值的盘中对照样本。
- 已继续把 near-miss 晋级从“可执行”推进到“值不值得优先保留”的结果闭环：新增 `scripts/analyze_targeted_short_trade_near_miss_release_outcomes.py` 与 `scripts/analyze_targeted_short_trade_near_miss_release_pair_comparison.py`，并补上对应回归测试。真实报告 `targeted_short_trade_near_miss_release_outcomes_001309_20260329.{json,md}` 显示，`001309` 晋级后的 2 个目标样本都保持正的收盘延续，给出 `next_high_return_mean=0.0510`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0`；而 `targeted_short_trade_near_miss_release_outcomes_300620_20260329.{json,md}` 则显示，`300620` 晋级后虽仍有 `next_high_return_mean=0.0479`，但 `next_close_return_mean=-0.0014`、`next_close_positive_rate=0.5`，已更像盘中对照。最终的成对对照 `targeted_short_trade_near_miss_release_pair_comparison_001309_vs_300620_20260329.{json,md}` 把结论固定为：`001309` 是 near-miss 晋级的第一优先入口，因为它既需要更少的阈值放松，也有更稳定的收盘延续；`300620` 继续保留为更激进的盘中晋级对照样本。最新新增的 `scripts/analyze_case_based_short_trade_entry_readiness.py` 与真实报告 `case_based_short_trade_entry_readiness_20260329.{json,md}` 又把这组结论推进成实验分层：`001309` 被正式标记为 `primary_controlled_follow_through`，`300383` 被标记为 `secondary_shadow_entry`，`300620` 则固定为 `control_only`。在此基础上，`scripts/analyze_case_based_short_trade_follow_through_runbook.py` 与真实报告 `case_based_short_trade_follow_through_runbook_20260329.{json,md}` 已继续把这三层角色落成可执行运行手册：`001309` 作为主实验入口固定使用 `select_threshold=0.56`，并以 `changed_non_target_case_count=0`、`next_close_return_mean>0`、`next_close_positive_rate>=0.75` 作为保留护栏；`300383` 固定为 `near_miss_threshold=0.42` 的影子入场；`300620` 只保留为盘中控制。也就是说，下一轮已经不需要再先设计一轮个案实验框架，而是可以直接按运行手册执行。
