...
> 决策摘要：关于“短线是否还应继续共用 Layer B 股票池”的短版结论，见 [short_trade_layer_b_split_decision_summary_20260329.md](./short_trade_layer_b_split_decision_summary_20260329.md)。
> 下一轮优先级摘要：关于“layer_c_bearish_conflict 是否应窗口级放松”的短版结论，见 [layer_c_bearish_conflict_release_queue_summary_20260329.md](./layer_c_bearish_conflict_release_queue_summary_20260329.md)。
...
- 已继续补齐 short trade score construction 定点诊断：`selection_target_score_diagnostics_300394_300502.json` 现可直接输出 replay 后的 `weighted_positive_contributions`、`weighted_negative_contributions`、`total_positive_contribution`、`total_negative_contribution` 与 thresholds。真实样本显示，`300394` 仍有较强正贡献（`trend_acceleration=0.1325`、`close_strength=0.1319`、总正贡献 `0.4281`），但被 `layer_c_avoid_penalty=0.12`、`stale_trend_repair_penalty=0.0564`、`extension_without_room_penalty=0.036` 明显压制；`300502` 的总正贡献仅 `0.1919`，低于总负贡献 `0.2254`，且 `breakout_freshness=0`、`volume_expansion_quality=0`。这把后续优化进一步拆成两类：`300394` 更像 penalty / score construction 问题，`300502` 更像 candidate entry 与 breakout 语义本身不匹配的问题。
- 已继续补齐 penalty weight 结构变体验证：`selection_target_penalty_weight_variants.json` 对比了 `no_bearish_conflict_block` 与 `no_bearish_conflict_softer_penalty_weights` 两个真实 replay 变体。结果显示，`300394` 的 `replayed_score_target` 从 `0.2133` 抬升到 `0.3207`，gap_to_near_miss 从 `0.2467` 收窄到 `0.1393`；`300502` 仅从 `0.0` 抬升到 `0.0792`，仍距离 near-miss 线 `0.3808`。这说明“降低 avoid/stale/extension 的 score 权重”对 `300394` 这类 penalty 主导样本有效，但不足以把它直接推到 near-miss；而 `300502` 依旧不是 penalty 调参对象，candidate entry / breakout 语义仍然是更高优先级。
- 已继续补齐 split penalty 与 candidate entry 聚焦诊断：`selection_target_penalty_split_variants.json` / `.md` 进一步把 `no_bearish_conflict_block` 拆成 `lower_avoid`、`lower_stale`、`lower_extension`、`lower_avoid_plus_stale`、`penalty_triplet_relief` 等真实变体；结果显示 `300394` 的单项最大收益来自 `layer_c_avoid_penalty`（`0.2133 -> 0.2733`），其次是 stale（`0.2415`）和 extension（`0.2313`），组合变体最高到 `0.3195`，与 softer penalty weights 的 `0.3207` 基本一致。与此同时，`selection_target_candidate_entry_focus.json` / `.md` 已证明 `300394`、`300502` 两个样本的 `candidate_source` 都是 `watchlist_filter_diagnostics`，原因码均为 `decision_avoid` 与 `score_final_below_watchlist_threshold`；这意味着 `300502` 的更高优先级已不再是 penalty 微调，而是审查 `watchlist_filter_diagnostics -> short trade replay candidate` 的入口语义是否过宽。
- 已继续补齐 candidate entry 过滤变体验证：`selection_target_candidate_entry_filter_variants.json` / `.md` 新增了排除 `watchlist_filter_diagnostics` 中 `decision_avoid + score_final_below_watchlist_threshold` 边界样本的 replay 变体。真实 `2026-03-26` 样本表明，这条规则会同时移除 `300394` 与 `300502`，把它们从 `blocked` 直接变成 `none`。这说明入口过滤方向本身是有效的，但当前规则粒度过粗，不能直接作为生产默认逻辑，因为它会误伤 `300394` 这类仍有较强正贡献、只是在 penalty 上受压的样本；因此下一轮 candidate entry 收紧应进一步叠加 breakout_freshness / volume_expansion_quality / catalyst_freshness 等正向结构条件，而不是只按 `avoid + 低分边界` 粗暴排除。
- 已继续补齐选择性弱结构 candidate entry 过滤验证：`selection_target_candidate_entry_selective_filter_variants.json` / `.md` 新增了 `exclude_watchlist_avoid_weak_structure_entries` 与 `no_bearish_conflict_exclude_watchlist_avoid_weak_structure_entries` 两个 replay 变体，在 `watchlist_filter_diagnostics + decision_avoid + score_final_below_watchlist_threshold` 的基础上进一步要求 `breakout_freshness <= 0.05`、`volume_expansion_quality <= 0.05`、`catalyst_freshness <= 0.05`。真实 `2026-03-26` 样本表明，该规则只会移除 `300502`，而会保留 `300394`；在 baseline 下结果从 `rejected=7, blocked=2` 变成 `rejected=7, blocked=1, none=1`，在 `no_bearish_conflict_block` 下则表现为 `300394` 继续作为 penalty 主导样本进入 `rejected`，`300502` 直接 `none`。这说明 candidate entry 收紧已经可以从“方向正确但过粗”推进到“具备样本区分力”的弱结构过滤原型，后续应优先把这类正向结构条件继续扩成更稳健的入口语义设计。
- 已继续补齐选择性弱结构 candidate entry 的 W1 长窗口方法学校验：`scripts/replay_selection_target_calibration.py` 新增 candidate-entry metric grid 能力与 `--breakout-freshness-max-grid`、`--volume-expansion-quality-max-grid`、`--catalyst-freshness-max-grid` 三组 CLI 参数，并在 `data/reports/paper_trading_window_20260202_20260313_w1_frozen_replay_m2_7_dual_target_replay_input_validation_20260329/selection_target_candidate_entry_metric_grid_w1.json` / `.md` 上完成 54 行网格回放。首轮实验暴露旧 frozen replay 源会因缺失 `strategy_signals` 把弱结构指标退化成零，从而误过滤 27 个 blocked entry；随后已补上“仅当 short-trade data gate 通过时才允许 metric-based candidate-entry filter 生效”的保护，并新增回归测试。保护生效后，W1 网格所有 54 行都回到 `filtered={}`、`mismatches=0`、`replayed={'blocked': 50}`；增强后的 eligibility 统计进一步显示，整个窗口里共有 27 个 entry 命中了 `watchlist_filter_diagnostics + decision_avoid + score_final_below_watchlist_threshold` 这组入口预条件，但 27 个全部停在 `metric_data_fail_count`，没有任何一个进入弱结构阈值比较。换句话说，这轮结果应被解释为“方法学边界已澄清，当前 W1 样本暂无可供该弱结构规则命中的高保真 entry”，而不是把 candidate entry 收紧方向本身否定掉。
- 已继续补齐 `300502` 的 candidate-entry semantic frontier：`scripts/replay_selection_target_calibration.py` 现已把 candidate-entry metric grid 扩展为可扫描 `--trend-acceleration-max-grid`、`--close-strength-max-grid`，并支持 `focus_tickers` / `preserve_tickers` 的最小分离行汇总。真实 `2026-03-26` 高保真样本上，新产物 `selection_target_candidate_entry_semantic_frontier_300502_refined.json` / `.md` 给出的最小 preserving row 为 `trend_acceleration <= 0.34` 且 `close_strength <= 0.69`：它会把 `300502` 从 replay 候选中直接过滤为 `blocked -> none`，同时保留 `300394` 不被过滤。对应的 filtered metrics 也直接说明两者差异：`300502` 只有 `trend_acceleration=0.3374`、`close_strength=0.6883`，而 `300394` 仍有 `trend_acceleration=0.7362`、`close_strength=0.942`。这说明 `300502` 的问题不只是 breakout/volume/catalyst 为零，还可以被量化为“短线趋势确认和收盘强度都不达标”的 candidate-entry 语义样本；下一轮入口规则可以继续沿这条正向结构分离面外推，而不必回到 penalty 路线。
- 已继续补齐 `300502` 的 candidate-entry subset frontier：candidate-entry metric grid 现已支持在 CLI 中使用 `none` 省略单个 metric 维度，从而直接搜索“最小子集规则”，而不是把 breakout、trend、volume、close、catalyst 拆成多轮人工实验。真实 `2026-03-26` 样本上的新产物 `selection_target_candidate_entry_subset_frontier_300502.json` / `.md` 显示，真正的最小 preserving row 甚至比 `trend_acceleration + close_strength` 联动更简单：仅要求 `volume_expansion_quality <= 0.0`，就已经足以把 `300502` 从 replay 候选中过滤为 `blocked -> none`，同时保留 `300394` 不被过滤。这个结果把 `300502` 的入口语义进一步压缩成一个更可操作的核心锚点：它首先是一个 zero-volume-expansion 样本，后续若需要做更稳健的生产规则，应优先以这个锚点为核心，再视需要叠加 breakout、trend 或 close 条件。
- 已继续补齐 replay 输入契约与跨日稳定性边界：`scripts/replay_selection_target_calibration.py` 现已支持直接读取 `selection_target_replay_input.json`、`selection_snapshot.json`、report 目录与 `selection_artifacts/` 目录，并新增 `selection_snapshot.json` 输入回归测试，使校准脚本在旧窗口缺少专用 replay input 时仍能复用高保真 snapshot。基于这一补强，已在新目录 `data/reports/paper_trading_window_20260323_20260326_live_m2_7_dual_target_replay_input_validation_20260329/` 上对 `2026-03-23` 与 `2026-03-24` 两个新鲜 dual-target snapshot 复用 `volume_expansion_quality <= 0.0` 的 volume-only replay：`2026-03-23` 中 `300502` 与 `300394` 虽可 replay，但都落在 `layer_b_boundary` 路径，`300502` 的 `volume_expansion_quality=0.25`、`300394` 为 `0.0`，该规则不会过滤任一 focus ticker；`2026-03-24` 则进一步显示当天 replay 汇总已是 `rejected=7`、`blocked=0`，focus ticker 仍然都是 `layer_b_boundary` 且 eligibility 为空，volume-only 规则没有任何可作用的 entry。结论因此需要收紧：`volume_expansion_quality <= 0.0` 目前只能解释为 `2026-03-26` 这类 candidate-entry 样本的单日最小 separating row，而不是已经证明可跨日复用的稳定规则。
- 已把分析视角从单 ticker 分治推进到窗口级失败机制扫描：`scripts/analyze_short_trade_blockers.py` 现已补充 `failure_mechanism_counts`、`candidate_source_breakdown` 与 `recommended_focus_areas` 输出，并新增对应回归测试。对当前 4 日高保真目录 `data/reports/paper_trading_window_20260323_20260326_live_m2_7_dual_target_replay_input_validation_20260329/` 运行后，`short_trade_blocker_analysis_current_window.json` / `.md` 给出两个清晰结论：其一，32 个 short-trade 样本里有 23 个直接停在 `layer_b_boundary` 且全部表现为 `rejected_layer_b_boundary_score_fail`，构成当前窗口里最大的失败簇；其二，另外 5 个样本被 `layer_c_bearish_conflict` 直接阻断，其中包括接近 near-miss 的 `300724` 与既有 penalty 主导样本 `300394`。这使主线优先级发生了实质收敛：下一轮最值得投入的不是继续放大 `300502` 的单样本 candidate-entry 规则，而是先做 `layer_b_boundary` 的 score construction / threshold 设计复盘，再做 `layer_c_bearish_conflict` 的结构性审查。
- 已继续把这两个主线方向各自落成专用诊断器，而不是停留在总量统计：新增 `scripts/analyze_layer_b_boundary_failures.py` 与 `scripts/analyze_structural_conflict_blockers.py`，并补上对应回归测试。对同一 4 日高保真目录运行后，`layer_b_boundary_failure_analysis_current_window.json` / `.md` 进一步证明 23 个 `layer_b_boundary` 样本虽然 `score_b` 均值已到 `0.3428`，但 short-trade `score_target` 均值只有 `0.1323`，距 near-miss 均值仍差 `0.3277`；其 strongest positive metrics 依次是 `close_strength=0.0841`、`trend_acceleration=0.0568`、`layer_c_alignment=0.0475`，而 `catalyst_freshness=0.0006`、`volume_expansion_quality=0.0062`、`breakout_freshness=0.0134` 几乎整体塌陷，说明主问题不是“阈值差一点”，而是边界候选在进入 short-trade 评分前就缺少可交易 breakout / catalyst 质量。另一方面，`structural_conflict_blocker_review_current_window.json` / `.md` 则把 `layer_c_bearish_conflict` 簇的主审查对象进一步缩小：5 个 blocked 样本里，最值得优先审的是 `2026-03-25 / 300724`，其 `score_target=0.3785`，距 near-miss 仅 `0.0815`；整个 blocked 簇的平均 penalty 暴露集中在 `overhead_supply_penalty=0.4682`、`stale_trend_repair_penalty=0.4606`、`extension_without_room_penalty=0.4497`。这两个专用诊断把下一轮真正应该改的策略面收敛为两条：第一，提升 `layer_b_boundary` 候选进入 short-trade 前的质量，而不是直接放宽 short-trade 阈值；第二，审查 `layer_c_bearish_conflict` hard block 是否与高 penalty 暴露发生重复惩罚，优先从 `300724` 这类高分 blocked 样本开始。
- 已把上述方向进一步落成最小代码改造，而不是继续停留在诊断层：`src/execution/daily_pipeline.py` 现已为 short-trade supplemental candidates 增加专属 boundary prefilter，只有同时满足 `breakout_freshness >= 0.18`、`trend_acceleration >= 0.22`、`volume_expansion_quality >= 0.15`、`catalyst_freshness >= 0.12` 且未触发 structural/data gate 的候选，才会从 Layer B 边界补入 short-trade；对应候选来源也显式标记为 `short_trade_boundary`，而不再在语义上与 generic `layer_b_boundary` 混为一谈。相关回归测试已覆盖强结构 boundary 样本保留、弱结构 boundary 样本被入口挡下，以及分析脚本对新旧来源的兼容。
- 已补做当前 4 日高保真窗口的静态影响验证，用来回答“新 prefilter 会不会真正打到当前最大失败簇”：由于 `frozen_plan_source` 复跑不会重算 post-market candidate generation，不能直接作为这类入口改动的验证器，因此改为对 `selection_snapshot.json` 中全部 23 个 `layer_b_boundary + rejected` 样本直接套用新 floor。结果显示 23/23 全部会在入口被拦下，其中 19 个首先死于 `breakout_freshness` 下限，3 个死于 `catalyst_freshness` 下限，1 个死于 `volume_expansion_quality` 下限；连当前该簇里分数最高的 `300502`、`300308`、`300274` 也都因为 catalyst / volume 结构缺失无法穿过 prefilter。结论因此比原先更清晰：短线和研究不应继续共享同一套 Layer B boundary 补充池，短线需要从入口就使用自己的结构条件，而不是把 research 风格的边界样本喂进来再在下游统一判死刑。
- 已继续把 short-trade boundary 从“独立 prefilter”推进成“独立 candidate builder”：`src/targets/short_trade_target.py` 现已抽出可复用的短线指标快照构建逻辑，`src/execution/daily_pipeline.py` 不再按 `FAST_AGENT_SCORE_THRESHOLD - buffer` 预选 boundary 候选，而是对全部非 watchlist 的 fused 候选直接计算 short-trade candidate score，并按 `0.30*breakout + 0.25*trend + 0.20*volume + 0.15*catalyst + 0.10*close` 排序后取前 N。这样短线补充池首次真正独立于 research 风格的 fast-score 邻域；对应回归测试已证明，低于旧 score buffer 的强短线结构样本也可以被补入 short trade，而空信号/弱结构样本会继续在入口被拦下。
- 已继续把这次建池改造推进到真实 live candidate-generation 路径验证，而不再只靠静态 snapshot 推断：为 `scripts/analyze_short_trade_blockers.py` 与 `scripts/analyze_layer_b_boundary_failures.py` 新增 `trade_dates` 过滤能力后，已把旧高保真报告与新 live partial 报告统一收敛到 `2026-03-23,2026-03-24` 两天做同口径对比。结果非常明确：旧报告在这两天共有 14 个 short-trade targets，其中 11 个来自 `layer_b_boundary` 且全部是 `rejected_layer_b_boundary_score_fail`，`layer_b_boundary` 的 `score_target` 均值只有 `0.1469`；新 live partial 报告则只剩 5 个 short-trade targets，且 `layer_b_boundary` rejection 直接降为 `0`，取而代之的是 2 个 `short_trade_boundary` 候选，它们都进入了 `near_miss`，`score_target` 均值达到 `0.5487`。这说明 short-trade 独立 candidate builder 不只是语义拆分成功，而是在真实 post-market 路径中已经把旧的共享 boundary 失败簇显著压缩，并把保留下来的 boundary 候选质量整体抬升到 near-miss 邻域。
- 已继续补完 `2026-03-25` 与 `2026-03-26` 两天的真实 live 验证，并把四天窗口证据收口成完整闭环，而不再停留在 partial live：`2026-03-25` 的旧基线报告里有 9 个 short-trade targets，其中 6 个来自 `layer_b_boundary` 且全部 score-fail，`layer_b_boundary` 的 `score_target` 均值只有 `0.1346`；对应的新 live builder 报告只剩 5 个 short-trade targets，`layer_b_boundary` rejection 下降到 `0`，同时出现 2 个 `short_trade_boundary` near-miss，均值 `score_target=0.5579`。`2026-03-26` 也呈现同方向结果：旧基线仍有 6 个 `layer_b_boundary` score-fail，均值 `0.1030`；新 live builder 报告同样把这部分 rejection 清零，并产生 2 个 `short_trade_boundary` near-miss，均值 `score_target=0.5372`。把 `2026-03-23` 到 `2026-03-26` 四天合并看，旧路径累计 23 个 `layer_b_boundary` score-fail，而新真实 live 路径里这类 rejection 已降到 `0`，替换为 6 个 `short_trade_boundary` near-miss。至此可以把结论收紧为：short-trade 从 Layer B 开始独立建池不是局部样本偶然，而是在完整 4 日窗口内都稳定地消除了旧共享 boundary 的主失败簇；剩余主矛盾也因此更清楚地收敛到 `layer_c_bearish_conflict` blocked 簇，而不再是 boundary candidate 质量塌陷。
- 已继续把“候选质量是否真的变好”从静态因子判断推进到次日真实表现对比：新增 `scripts/analyze_pre_layer_short_trade_outcomes.py` 与对应回归测试，可直接从 `selection_target_replay_input.json` 读取 Layer C 之前的 short-trade supplemental 候选，并统计其 T+1 `next_open_return`、`next_high_return`、`next_close_return`。对真实旧窗口 `layer_b_boundary` 与新 builder 窗口 `short_trade_boundary` 做同口径比较后，结果显示：旧池 `23` 个样本的 `next_high_return_mean=0.0263`、`next_close_return_mean=0.0027`、`next_high_hit_rate@2%=0.5217`、`next_close_positive_rate=0.5652`；新池当前 `2` 个样本的 `next_high_return_mean=0.0829`、`next_close_return_mean=0.0498`、`next_high_hit_rate@2%=1.0`、`next_close_positive_rate=1.0`。这使 Layer C 之前的主线判断再次收紧：新 `short_trade_boundary` builder 已明显提升候选质量，下一轮更值得做的是扩覆盖而不是继续收紧过滤。
- 已继续把“扩覆盖”从方向判断推进成可验证的 floor 变体实验与过滤明细观测：新增 `scripts/analyze_short_trade_boundary_coverage_variants.py`、`scripts/analyze_short_trade_boundary_filtered_candidates.py` 及对应回归测试。真实 `2026-03-23` 到 `2026-03-26` 旧 `layer_b_boundary` 候选池上的 baseline 回放显示，当前 23 个样本在默认 short-trade boundary floor 下 `0` 个可放行，其中 `19` 个先死于 `breakout_freshness`、`3` 个死于 `catalyst_freshness`、`1` 个死于 `volume_expansion_quality`，说明大部分旧池样本并不是“轻微差一点”。过滤明细进一步显示，最接近放行的 edge candidate 主要集中在 `300308`、`300274`、`300502` 这类只差 `catalyst_freshness=0.12` 的样本。围绕这簇样本做定向变体后，若仅把 `catalyst_freshness_min` 从 `0.12` 放到 `0.00`，其余 floor 保持不变，可新增 `3` 个样本，且 `next_high_return_mean=0.0222`、`next_close_return_mean=0.0028`、`next_high_hit_rate@2%=0.6667`、`next_close_positive_rate=0.6667`；若再把 `volume_expansion_quality_min` 一并降到 `0.06`，样本虽增到 `4` 个，但 `next_close_return_mean` 已回落到 `-0.0036`、`next_close_positive_rate` 降到 `0.5`。这把下一轮前置主线进一步收紧为：如要扩 coverage，优先做 `catalyst_freshness_min=0.00` 的受控实验，不要先联动放松 volume floor；breakout floor 暂时也不应被当作“轻微扩覆盖”的主杠杆，因为当前 `19/23` 的 breakout 失败更像结构性缺失而不是边缘样本。
- 已继续把这条 catalyst-only 扩覆盖建议推进到真实 live candidate-generation 路径，而不是停留在静态 replay：新增 `scripts/analyze_short_trade_boundary_expansion_cases.py` 与 `scripts/run_short_trade_boundary_variant_validation.py` 及对应回归测试，并启动 `DAILY_PIPELINE_SHORT_TRADE_BOUNDARY_CATALYST_MIN=0.0` 的真实 live paper trading 变体。现在完整 `2026-03-23` 到 `2026-03-26` 四天窗口已经全部收口：full-window blocker 报告显示 short-trade target 总数为 `33`，其中 `24` 个来自 `short_trade_boundary`，`near_miss=6`、`rejected_short_trade_boundary_score_fail=18`，且旧 `layer_b_boundary` score-fail 簇依旧保持为 `0`；full-window pre-Layer C outcome 报告则显示这 `24` 个候选的 `next_high_return_mean=0.0471`、`next_close_return_mean=0.0186`、`next_high_hit_rate@2%=0.75`、`next_close_positive_rate=0.7083`，仍明显强于旧 `layer_b_boundary` 池。同时，新增的逐票案例报告已确认静态 targeted coverage 推荐变体释放的 `300308`、`300274`、`300502` 都属于纯 `catalyst_freshness` 单条门槛放行；而新补的 `scripts/analyze_short_trade_boundary_score_failures.py` 与 `scripts/analyze_short_trade_boundary_score_failures_frontier.py` 则进一步证明，这批 `18` 个 `short_trade_boundary` score-fail 样本并不是新的 admission 垃圾簇，而是 breakout/trend/volume/close 结构已过线、但大多仍距 near-miss `0.06+` 的 score frontier 样本。frontier 扫描显示 `18/18` 都存在 near-miss rescue row，但仅 `1/18`（`2026-03-26 / 300383`）属于 threshold-only 低成本释放，其余都需要 stale/extension penalty 联动下调。这把下一轮前置主线再次收紧为：默认保留 `catalyst_freshness_min=0.00` 作为已通过完整窗口验证的 admission 变体，并把后续优化重心切到 `short_trade_boundary` score frontier，而不是继续寻找第二条 admission floor。
- 已继续把 `short_trade_boundary` score frontier 从“窗口级建议”推进成可执行的 case-based 与 recurring-ticker 两层实验入口：新增 `scripts/analyze_targeted_short_trade_boundary_release.py` 与 `scripts/analyze_short_trade_boundary_recurring_frontier_cases.py` 及对应回归测试。真实 `300383` 定向 release 实验已确认，只把 `near_miss_threshold` 从 `0.46 -> 0.42` 就能让 `2026-03-26 / 300383` 从 `rejected -> near_miss`，且其余 `17` 个 rejected `short_trade_boundary` 样本完全不变；这说明它已经具备低污染的 case-based release 语义。与此同时，recurring frontier 队列也把重复样本收敛为 `600821` 与 `002015` 两只，其中 `600821` 在 `2026-03-23, 2026-03-25, 2026-03-26` 三天重复出现，最小 rescue cost 为 `0.10`，`002015` 则在 `2026-03-23, 2026-03-24, 2026-03-25` 三天重复出现，最小 rescue cost 为 `0.12`；两者都不属于 threshold-only，而是需要 stale/extension penalty 联动下调。随后又把 `scripts/analyze_pre_layer_short_trade_outcomes.py` 扩展为支持 `--tickers` 定向过滤，并新增 `scripts/analyze_targeted_short_trade_boundary_release_outcomes.py`、`scripts/analyze_short_trade_boundary_recurring_frontier_dossiers.py`、`scripts/analyze_short_trade_boundary_frontier_ticker_dossier.py`、`scripts/analyze_short_trade_boundary_frontier_pair_comparison.py`、`scripts/analyze_recurring_frontier_ticker_release.py`、`scripts/analyze_recurring_frontier_ticker_release_outcomes.py`、`scripts/analyze_recurring_frontier_release_pair_comparison.py`、`scripts/analyze_short_trade_ticker_role_history.py`、`scripts/analyze_recurring_frontier_transition_candidates.py`、`scripts/analyze_multi_window_short_trade_role_candidates.py`、`scripts/analyze_multi_window_short_trade_ticker_dossier.py` 与 `scripts/analyze_multi_window_short_trade_ticker_pair_comparison.py`，把 release/frontier 结果与真实次日表现合并成固定 dossier、release baseline、跨窗口角色迁移证据，以及“local baseline vs stable rule”的正式分类器。最新一轮又补上了自动发现历史窗口的能力，可直接从 `data/reports` 根目录递归发现带 `selection_snapshot.json` 的 `paper_trading_window_*` 报告；基于这项能力生成的 `recurring_frontier_transition_candidates_all_windows_20260329.md` 已覆盖 `14` 个自动发现窗口，且结论保持不变。随后新增的全局扫描 `multi_window_short_trade_role_candidates_20260329.{json,md}` 又把边界进一步明确：自动发现窗口范围内共有 `39` 个曾进入 short-trade 角色的 ticker，但只有 `4` 个 ticker 满足“至少 2 个 short-trade trade_date”，且 `stable_candidate_count = 0`；这 4 个 ticker 分别是 `600821`、`002015` 以及当前窗口内新出现的 near-miss 型样本 `001309`、`300620`，四者全部都仍属于 `emergent_local_baseline`。真实 live admission 窗口补做 outcome 对照后，`300383` 单票样本的 `next_open_return=0.0246`、`next_high_return=0.0527`、`next_close_return=0.0146`，说明它不仅是低污染 release，也是正向次日表现样本；`600821` 与 `002015` 的 recurring dossier 则进一步拆出：`600821` 三次样本 `next_high_return_mean=0.0503`、`next_close_positive_rate=0.3333`，已可固定为 recurring frontier 的 intraday 主样本；`002015` 三次样本 `next_high_return_mean=0.0339`、`next_close_positive_rate=0.6667`，更适合作为 close continuation 对照样本。更进一步的 ticker-level recurring release 实验已确认，两只票都能在各自 recurring row 下实现 `3/3 rejected -> near_miss` 且 `changed_non_target_case_count=0`；release-outcome pair comparison 则把角色彻底固定为：`600821` 是 recurring release 的 intraday 主样本，`002015` 是 close-continuation 对照样本。对 near-miss emergent baseline 的新补 dossier 与 pair comparison 则进一步说明，`001309` 的两次可用样本给出 `next_high_return_mean=0.0510`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0`，应作为 near-miss close-continuation 优先样本；`300620` 的两次可用样本给出 `next_high_return_mean=0.0479`、`next_close_return_mean=-0.0014`、`next_close_positive_rate=0.5`，更适合作为 near-miss intraday 对照样本。最后，新补的 ticker role history 报告、transition scanner、全局 multi-window 扫描器与 near-miss dossier/pair comparison 又把边界进一步收紧：在更早的 `paper_trading_window_20260316_20260323_live_m2_7_20260323` 里，两只 frontier ticker 都还只是旧 Layer B 池里的 `below_fast_score_threshold` 样本，只是在当前 catalyst-only 窗口里才首次连续变成 recurring `short_trade_boundary_rejected`；而 `001309`、`300620` 也只是当前窗口的 `short_trade_boundary_near_miss` 型 emergent baseline，并没有跨窗口复现证据。因此它们现在都应被定义为“当前窗口内成立的局部 baseline”，而不是已经跨窗口稳定的历史规则。最新一轮新增的 `scripts/analyze_short_trade_release_priority_scoreboard.py` 与 `scripts/analyze_case_based_short_trade_entry_pair_comparison.py` 则把此前分散的 case-based 与 recurring 入口放到同一标尺上重新排序：`short_trade_release_priority_scoreboard_20260329.{json,md}` 给出的统一结论是 `001309` 第一、`300383` 第二、`002015` 第三、`600821` 第四；对应的 `case_based_short_trade_entry_pair_comparison_001309_vs_300383_20260329.{json,md}` 又进一步确认，`001309` 在 `adjustment_cost=0.02`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0` 这组指标上已经压过 `300383` 的单样本 threshold-only release。于是更准确的下一轮优先级应改写为：先做 `001309` 的 near-miss promotion follow-through，再做 `300383` 的 threshold-only release，而 `002015`、`600821` 则继续保留为 recurring frontier 的局部 baseline 对照。
- 已继续把 near-miss emergent baseline 从“观察样本”推进成受控 promotion 入口：新增 `scripts/analyze_targeted_short_trade_near_miss_release.py` 与对应回归测试，可对指定 `trade_date:ticker` 的 `short_trade_boundary` near-miss 样本执行定向 selected release 审查。真实窗口报告 `targeted_short_trade_near_miss_release_001309_20260329.{json,md}` 已确认，`001309` 在 `2026-03-24` 与 `2026-03-25` 两个 near-miss 样本上，只需把 `select_threshold` 从 `0.58 -> 0.56` 就能实现 `2/2 near_miss -> selected`，且 `changed_non_target_case_count=0`；对照报告 `targeted_short_trade_near_miss_release_300620_20260329.{json,md}` 则显示，`300620` 若要实现同样的 `2/2 near_miss -> selected`，需要把 `select_threshold` 进一步放到 `0.53`。这把 near-miss 旁路的内部优先级再次收紧为：`001309` 已可视为低污染、threshold-only 的 case-based promotion 样本，而 `300620` 仍更适合作为需要更激进阈值的 intraday 对照样本。
- 已继续把 near-miss promotion 从“可执行”推进到“值不值得优先保留”的 outcome 闭环：新增 `scripts/analyze_targeted_short_trade_near_miss_release_outcomes.py` 与 `scripts/analyze_targeted_short_trade_near_miss_release_pair_comparison.py`，并补上对应回归测试。真实报告 `targeted_short_trade_near_miss_release_outcomes_001309_20260329.{json,md}` 显示，`001309` promotion 后的 2 个目标样本都保持正的 close follow-through，给出 `next_high_return_mean=0.0510`、`next_close_return_mean=0.0414`、`next_close_positive_rate=1.0`；而 `targeted_short_trade_near_miss_release_outcomes_300620_20260329.{json,md}` 则显示，`300620` promotion 后虽仍有 `next_high_return_mean=0.0479`，但 `next_close_return_mean=-0.0014`、`next_close_positive_rate=0.5`，已更像 intraday 对照。最终的 pair comparison `targeted_short_trade_near_miss_release_pair_comparison_001309_vs_300620_20260329.{json,md}` 把结论固定为：`001309` 是 near-miss promotion 的第一优先入口，因为它既需要更少的 threshold 放松，也有更稳定的 close continuation；`300620` 继续保留为更激进的 intraday promotion 对照样本。最新新增的 `scripts/analyze_case_based_short_trade_entry_readiness.py` 与真实报告 `case_based_short_trade_entry_readiness_20260329.{json,md}` 又把这组结论推进成实验分层：`001309` 被正式标记为 `primary_controlled_follow_through`，`300383` 被标记为 `secondary_shadow_entry`，`300620` 则固定为 `control_only`。在此基础上，`scripts/analyze_case_based_short_trade_follow_through_runbook.py` 与真实报告 `case_based_short_trade_follow_through_runbook_20260329.{json,md}` 已继续把这三层角色落成可执行 runbook：`001309` 作为主实验入口固定使用 `select_threshold=0.56`，并以 `changed_non_target_case_count=0`、`next_close_return_mean>0`、`next_close_positive_rate>=0.75` 作为 keep guardrails；`300383` 固定为 `near_miss_threshold=0.42` 的 shadow entry；`300620` 只保留为 intraday control。也就是说，下一轮已经不需要再先设计一轮 case-based 实验框架，而是可以直接按 runbook 执行。
- 已继续把 `layer_c_bearish_conflict` 的 hard block 与 overhead conflict surcharge 从同一配置开关中拆开：`ShortTradeTargetProfile` 现已分别支持 `hard_block_bearish_conflicts` 与 `overhead_conflict_penalty_conflicts`，同时保留 `strong_bearish_conflicts` 作为兼容别名，默认行为保持不变。这一步的意义不是立即改默认策略，而是把“hard block 是否该放松”和“冲突 surcharge 是否该保留”变成可以独立验证的两条控制轴，避免后续继续把 300724 一类样本混在一个不可分解的黑盒里审。
- 基于这次解耦，已新增定点诊断脚本 `scripts/analyze_structural_conflict_rescue.py`，并在真实 `2026-03-25 / 300724` 样本上完成首轮 rescue 分析。结果非常清楚：baseline 下 `300724` 为 `blocked`，`score_target=0.3785`，距 near-miss 仍差 `0.0815`；若只移除 hard block 但保留 conflict penalty，decision 只会从 `blocked -> rejected`，分数完全不变；若同时移除 hard block 与 conflict surcharge，则 `score_target` 可升至 `0.4235`，gap_to_near_miss 收窄到 `0.0365`，但在默认阈值下仍是 `rejected`。只有在此基础上再采用更激进的 near-miss 阈值（`0.42`）时，它才会首次进入 `near_miss`。这说明 `300724` 的主问题不是“hard block 一拿掉就能自动救回”，而是 hard block 与 conflict surcharge 确实存在重复惩罚，但仅消除重复惩罚仍不足以让它在默认 short-trade 阈值下过线。
- 已继续把这条判断推进成可搜索 frontier，而不是停留在 4 个手工变体：`scripts/analyze_structural_conflict_rescue.py` 现已新增 penalty+threshold grid 能力，可在固定移除 conflict hard block 与 surcharge 的前提下，直接扫描 `stale_score_penalty_weight`、`extension_score_penalty_weight`、`select_threshold` 与 `near_miss_threshold` 的最小联动条件，并输出 `minimal_near_miss_row` / `minimal_selected_row` / `best_score_row`。真实 `2026-03-25 / 300724` 的 900 行 frontier 显示，最小 near-miss rescue row 实际上不需要再放松 stale 或 extension penalty：只要保持 `stale_score_penalty_weight=0.12`、`extension_score_penalty_weight=0.08` 不变，并把 `near_miss_threshold` 从 `0.46` 下调到 `0.42`，就已经能以总 adjustment_cost=`0.04` 进入 `near_miss`。但若目标是 `selected`，最小 row 则需要 `stale_score_penalty_weight=0.06`、`extension_score_penalty_weight=0.00`，并把 `select_threshold` 放宽到 `0.48`，总 adjustment_cost=`0.24`。这把 `300724` 的后续判断进一步收敛成两层：若只是释放到 near-miss，主杠杆仍是阈值；若想让它直接 selected，才需要 penalty weight 的实质联动放松。
- 已继续把单点 `300724` rescue 扩展成窗口级 blocked 簇排序，而不再只看单样本：新增 `scripts/analyze_structural_conflict_rescue_window.py`，可直接扫描整份 report 中所有 `layer_c_bearish_conflict` blocked 样本，并按 `minimal_near_miss_adjustment_cost` 输出 rescue priority queue。对当前 4 日高保真窗口运行后，结论进一步收紧：5 个 blocked 样本里，只有 `2026-03-25 / 300724` 在当前搜索空间内存在 near-miss rescue row，且最小 cost 仍然只有 `0.04`；同一窗口中的 `300394`（`2026-03-25`、`2026-03-26`）、`300308` 与 `300502` 都没有任何 near-miss row，说明它们不适合和 `300724` 一起被打包成“统一放松 hard block”实验。换句话说，`layer_c_bearish_conflict` 的下一轮最合理动作不是做 cluster-wide 放宽，而是把 `300724` 作为唯一低成本 near-miss 释放候选单独做受控实验，其余 blocked 样本继续留在 candidate-entry / score construction 路径。
- 已把上述建议继续推进成真实 case-based 验证，而不是停留在 frontier 层：新增 `scripts/analyze_targeted_structural_conflict_release.py` 与对应回归测试，可对指定 `trade_date:ticker` 执行定向 structural conflict release，并输出窗口级 before/after 决策变化。对当前 4 日高保真窗口执行 `2026-03-25:300724` 实验后，结果显示总样本 `32` 个里只有 `1` 个样本发生变化，即 `300724` 从 `blocked -> near_miss`，`score_target` 从 `0.3785` 抬升到 `0.4235`；其余 `31` 个样本完全不动，`300394`、`300308`、`300502` 均未被污染。这使下一轮策略建议进一步收紧为：如果要做 release 实验，优先做这种 case-based 的 `300724-only` 受控变体，而不是任何按 blocked 簇或整日全局放宽的版本。
- 已继续补齐 `300394` 的 penalty frontier 与最小 rescue 搜索：`scripts/replay_selection_target_calibration.py` 新增 `--avoid-penalty-grid`、`--stale-score-penalty-grid`、`--extension-score-penalty-grid`，以及 penalty+threshold 联合 frontier 分析能力；对应回归测试现已增至 `17 passed`。真实 `2026-03-26` 样本上的 180 行 penalty frontier 显示，即便在 `no_bearish_conflict_block` 下把 `layer_c_avoid_penalty` 压到 `0.02`、`stale_score_penalty_weight` 压到 `0.02`、`extension_score_penalty_weight` 压到 `0.00`，`300394` 的最高 `replayed_score_target` 也只有 `0.3963`，距离 `near_miss=0.46` 仍差 `0.0637`；`300502` 最高仅到 `0.1575`，距离 near-miss 仍差 `0.3025`。这说明“只放松 penalty”不足以把 `300394` 推进 near-miss，更不可能救回 `300502`。
- 已继续补齐 `300394` 的 penalty+threshold 最小 rescue 条件：新增命名变体 `no_bearish_conflict_extreme_penalty_relief` 后，组合扫描首先证明 `300394` 只有在极端 penalty relief 下并把 `near_miss_threshold` 下调到 `0.38` 时，才会首次出现 `blocked -> near_miss`；当 `select_threshold=0.38`、`near_miss_threshold=0.38` 时，才会首次 `blocked -> selected`。进一步的 1600 行 penalty+threshold 联合 frontier 则给出更细的最小方案：`300394` 的最小 near-miss rescue row 是 `layer_c_avoid_penalty=0.02`、`stale_score_penalty_weight=0.02`、`extension_score_penalty_weight=0.08`、`near_miss_threshold=0.36`、`select_threshold` 仍保持 `0.58`，总 adjustment_cost=`0.30`；最小 selected rescue row 则是 `layer_c_avoid_penalty=0.02`、`stale_score_penalty_weight=0.02`、`extension_score_penalty_weight=0.02`、`select_threshold=0.38`、`near_miss_threshold=0.38`，总 adjustment_cost=`0.54`。而 `300502` 在同一搜索空间里没有任何 near-miss/selected rescue row，说明它仍然不是 penalty/threshold 联动优先对象，而应继续留在 candidate entry / breakout 语义路径。
...
- 2026-03-28 已继续补齐 dual-target replay workspace 的第二、三轮可用性增强：report detail 现已带有 trade_date_target_index，前端可按 target mode / delta classification 过滤 trade date、显示匹配天数，并通过 Trade Date Target Index 面板直接跳回具体 trade_date；focused symbol 已联动 feedback 筛选、单条/批量 feedback 表单、Cross-Report Workflow Queue 排序与 inspector 摘要；workflow queue 现已支持 current report only / focus symbol only 的本地收窄；workflow queue、pending draft queue 与 latest activity 现已支持 Open Context，一键跳回对应 report / trade_date / symbol。
