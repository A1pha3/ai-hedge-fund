...
6. 同日已先后完成 `2026-02-02` 单日 probe、`2026-02-02..2026-02-03` 两日 rerun、`2026-02-02..2026-02-06` 五日 rerun与 `2026-02-02..2026-02-10` extended-window rerun，四者都显示 `providers_seen=["MiniMax"]`、`routes_seen=["MiniMax:default"]`；说明 session 级单 provider 闸门已在 `1d + 2d + 5d + extended-window` 窗口下成立，后续若继续推进这条线，应把它视为独立分支继续扩窗，而不是直接把它混入 W3 结论。
7. 其中 extended-window 虽然继续保持 `fallback_attempts=0` 与单 provider provenance，但同时暴露 `APIConnectionError=159`、`RateLimitError=23` 的稳定性压力；因此这档结果应联读 [single-provider-session-extended-window-stability-20260320.md](./single-provider-session-extended-window-stability-20260320.md)，不能被表述为 clean extended run。
...
6. 若继续推进 provider 隔离验证，则以 [single-provider-session-probe-20260320.md](./single-provider-session-probe-20260320.md) 为当前已验证起点，在 `1d -> 2d -> 5d -> extended-window` 已成立的前提下，再决定是否继续向更长窗口扩展，而不是直接把 single-provider 结论并入 W3。
...
9. [single-provider-session-extended-window-stability-20260320.md](./single-provider-session-extended-window-stability-20260320.md)
