...
| [研究结论到优化 Backlog 的转换手册](./manual/research-conclusion-to-optimization-backlog-handbook.md) | 把周度复盘里形成的研究结论标准化映射到 Layer B、Layer C、Execution、Threshold、Explainability 五类优化动作，减少“看懂了但不会落地”的断层 | 已完成周度复盘、需要把结论转成后续策略或工程动作的研究员、负责人和开发者 | ⭐⭐⭐ |
...
| [次日短线目标指标与验证方案](./product/arch/short_trade_target_metrics_and_validation.md) | 定义短线目标的 success metrics、执行真实性约束、验证窗口、regime 切片和默认档升级门槛，避免回测幻觉和主观调参 | 产品、开发者、研究者 | 按需更新 |
| [次日短线目标首版规则集规格](./product/arch/short_trade_target_rule_spec.md) | 把 short trade target 首版规则拆成输入事实、因子集、归一化、gate 顺序、explainability 和测试建议，便于后续实现与评审 | 产品、开发者、研究者 | 按需更新 |
| [双目标系统数据结构与 Artifact Schema 规格](./product/arch/dual_target_data_contract_and_artifact_schema.md) | 冻结双目标系统的目标层输入输出、ExecutionPlan 承接方式、selection_artifacts 扩展结构，以及 replay、summary、feedback 的字段级协议 | 产品、开发者、研究者 | 按需更新 |
| [双目标系统实施与代码改造计划](./product/arch/dual_target_implementation_plan.md) | 把双目标系统的实际施工顺序收敛为阶段化改造计划，明确代码落点、每阶段产物、回归要求和风险控制策略 | 产品、开发者、研究者 | 按需更新 |
...
| [Layer B 长期趋势因子默认开启验证](./analysis/layer-b-long-trend-default-on-validation-20260327.md) | 记录 long_trend_alignment 默认开启后与显式关闭对照的 202603 窗口结果，确认当前窗口无新增放量但默认开启是安全的 | 开发者、研究者 | 按需更新 |
...
| [专题分析目录](./analysis/README.md) | 运行分析、性能分析与专项设计说明 | 开发者、研究者 | 按需更新 |
