...
`--model` 用于指定使用的 **LLM（大语言模型）** 提供商。

**可用模型对比**：

| 模型 | 特点 | 适用场景 | 速度 | 成本 |
|------|------|---------|------|------|
| `openai` | 平衡性好，综合能力强 | 通用分析 | 中等 | 中等 |
| `anthropic` | 理解力强，推理深入 | 复杂分析 | 较慢 | 较高 |
| `groq` | 极速推理，成本低 | 快速批量分析 | 极快 | 低 |
| `deepseek` | 性价比高 | 成本敏感场景 | 较快 | 低 |
