...
适用对象：要亲手运行 BTST 回放、真实窗口验证、前沿分析和窗口复盘的研究员、开发者、AI 助手。
...
如果顺序打乱，最容易出现的问题是：先跑了很多前沿分析，但其实你连当前主失败簇都没确认。
...
1. 每条变体新增了几个样本。
2. 是不是只释放边缘高质量样本。
3. 是否应进入 live validation（真实窗口验证）。
...
1. 你要把 frontier（前沿）结论和真实次日表现合在一起看。
...
1. 你要把 closed-cycle baseline、已验证准入变体和 forward-only 样本放进同一套 BTST 框架里比较。
2. 你要明确当前问题到底是“可执行曲面”为空，还是可执行样本已经出现但仍有大量 false negative。
...
### 7.7 做 profile 前沿闭环比较

适用场景：

1. 你已经确认准入 baseline 不应再盲目放宽，但想验证“只调 short-trade profile 语义”能不能把窗口从 0 可执行曲面推起来。
2. 你要把 `default`、`staged_breakout`、`aggressive`、`conservative` 放到同一个 closed-cycle BTST outcome 面上比较。
3. 你要回答“仅调 profile 的变体是否已经足以形成闭环可执行曲面，还是仍应回到分数构造 / 候选入口主线”。
...
1. 哪个 profile 真的新增了闭环可执行曲面。
2. profile-only 变体是否通过了 baseline false negative proxy 派生出的 guardrails。
3. 如果所有 profile 都还是 0 可执行曲面，是否可以正式收敛到“下一步不再优先调 profile，而是回到分数前沿 / 候选入口”。

优先回答：

1. baseline 的“可执行曲面”是否仍为 0，以及 `false_negative_proxy_summary` 还有多大。
2. 变体是否真的新增了闭环可执行样本，而不只是把样本从 `rejected` 挪到 `near_miss`。
...
1. 你已经验证过准入 baseline 和仅调 profile 的变体都没有把窗口从 0 可执行曲面推起来。
2. 你要确认“只调正向 score weight 分配”是否真能形成闭环可执行曲面。
...
1. 哪个正向 weight 变体真的新增了闭环可执行曲面。
2. 如果所有 score 变体都仍是 0 可执行曲面，是否可以正式把仅调 score 的路线从主线里降级。
...
2. 这份结果若依旧是全 0 可执行曲面，就说明当前问题不能再理解成“只要重配正向分数权重就能推起曲面”。

### 7.9 做候选入口前沿闭环比较

适用场景：

1. 你已经确认仅调 score 的路线仍没有形成可执行曲面。
2. 你要验证选择性候选入口语义，看看能不能过滤掉 300502 这类弱结构 watchlist_avoid 样本，同时保住 300394 这类 preserve 样本。
...
1. 哪条候选入口规则能过滤 focus ticker，而不误伤 preserve ticker。
...
1. 你已经在当前窗口确认 `weak_structure_triplet` 是最强的候选入口选择性规则。
...
3. 由于 `preserve_misfire_report_count=0`，这条规则可以继续保留为候选入口影子旁路，但不能升级为默认准入行为。

### 7.11 生成候选入口推进治理板

适用场景：

1. 你已经有 current-window candidate-entry frontier（当前窗口候选入口前沿）结果。
2. 你已经用 structural variant 主链回放验证过 `exclude_watchlist_avoid_weak_structure_entries`。
3. 你需要把单窗前沿、多窗扫描和分数前沿零结果收口成一个明确的治理结论。
...
1. 当前 `candidate_entry_rule` 应该固定成哪条候选入口语义，以及对应哪条结构变体。
2. 当前车道状态是否只允许“第二独立窗口前仅影子观察”（`shadow_only_until_second_window`）。
3. 为什么现在不能把候选入口规则误写成默认升级依据。
...
2. 跑 `analyze_short_trade_boundary_score_failures.py` 与 frontier（前沿）版本，确认 score fail 主簇。
3. 跑 `analyze_btst_score_construction_frontier.py`，验证正向 weight 微调是否真能形成 closed-cycle actionable surface（闭环可执行曲面）。
