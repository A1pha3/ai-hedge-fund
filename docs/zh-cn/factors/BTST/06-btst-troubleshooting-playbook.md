...
适用对象：在 BTST 真实窗口、回放校准或研究复盘中遇到异常结果，想快速判断问题落在哪一层的研究员、开发者和 AI 助手。
...
### 2.2 第 2 层：short trade boundary 准入
...
典型问题：准入已经过了，但 selected / near_miss 很少。
...
| --- | --- | --- | --- |
| BTST 候选总体很少 | Layer B 供给 / 准入 | `layer_b_count`、boundary candidate count | 先查上游供给，再查准入 |
| boundary 候选几乎为 0 | 准入 | `filtered_reason_counts` | 先看哪条门槛卡得最多 |
...
| 某个 ticker 总差一点 | 聚焦个股前沿 | focused score diagnostics | 看最小救援行 |
| selected 增多但次日变差 | execution / 质量劣化 | next-day outcomes | 回滚本轮变体，重新评估准入 |
| 回放变好但真实窗口没变好 | 验证错位 | live report、next-day outcomes | 以真实窗口结果为准 |
...
1. 准入变体优先，不先动 target profile。

### 场景 3：大量 `rejected_short_trade_boundary_score_fail`

why：说明准入通过后，正式评分仍压死了大批样本。
...
1. 惩罚前沿，必要时再加 threshold。

### 场景 9：回放里能 near-miss，真实窗口里没改善

why：可能是回放输入和真实盘后供给不一致。
...
| 问题类型 | 脚本 | 目标 |
| --- | --- | --- |
| 回放决策漂移 | `replay_selection_target_calibration.py` | 先看是不是规则漂移 |
| 前置候选质量 | `analyze_pre_layer_short_trade_outcomes.py` | 看次日质量 |
| score fail 主因 | `analyze_short_trade_boundary_score_failures.py` | 看主失败簇 |
| near-miss rescue | `analyze_short_trade_boundary_score_failures_frontier.py` | 找最小成本救援行 |
| blocked 释放优先级 | `analyze_structural_conflict_rescue_window.py` | 看谁值得先救 |
| 定点 blocked 实验 | `analyze_targeted_structural_conflict_release.py` | 做个案释放 |
...
BTST 排障最重要的不是“知道哪些参数能改”，而是先判断问题到底落在哪一层；只有层级判断正确，后续回放、前沿分析、真实窗口验证和默认值升级才不会跑偏。
