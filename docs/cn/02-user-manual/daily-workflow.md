...
- 运行时机:`--auto` 完成后,当日 17:00 之后(17:00 guard 切换信号日)
...
### 17:00 后运行 `--daily-action`

```bash
uv run python src/main.py --daily-action
```

读取 `--auto` 缓存的 price_cache 与 regime 数据,扫描 BTST 涨停突破 setup。17:00 guard 在此时切换信号日:当前时间 ≥ 17:00 时,signal_date = 今日(20260713),计划买入日 = 下一交易日(20260714 周二)开盘。

> 17:00 之前跑也能拿到信号,但 signal_date 会回退到上一交易日(用于盘前研究当日盘面),不是次日信号。要拿次日 BUY 信号必须在 17:00 后跑。
...
`--daily-action` 的 17:00 guard 决定 signal_date:≥ 17:00 用当日信号(产出次日 BUY),< 17:00 回退到上一交易日(用于盘前研究)。如果需要补跑历史日期,用 `--end-date` 显式覆盖:
...
| 晚间 | 17:00 后 | `--daily-action` | ~3 秒 | 次日 BUY 信号 |
| 次日开盘 | 09:30 | (手动执行) | - | 买入 |
| 窗口关闭 | 次日 17:00 | - | - | 信号失效 |
