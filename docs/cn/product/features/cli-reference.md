...
- 不带日期参数时，`--auto` 与 `--daily-action` 都不会再生成周六、周日这种伪交易日；它们会先应用 `17:00` 数据就绪规则，再统一回退到最近开市日。
- 周日运行：两个命令默认都会落到上周五的交易日数据。
- 周一 `17:00` 前运行：默认仍使用上周五，因为周一收盘后的资金流和全量数据还没就绪。
- 周一 `17:00` 后运行：默认切到周一，当晚的 `--auto` 会生成周一报告，随后 `--daily-action` 会基于周一信号生成周二开盘计划。
- `--daily-action` 的 stale guard 现在比较的是“最新报告对应的交易日”，不是报告生成时的自然日；因此周末跑过 `--auto`，不会再让周一盘前误报“数据滞后”。
...
> 以下 **36 个环境变量**用于运行时调优与实验开关，全部带缺省值、可不配置即可运行。**新手配置请看 [`/.env.example`](../../../.env.example)**；本节仅收录进阶调参项。阈值语义以代码常量定义为准（见各表"代码"列）。
