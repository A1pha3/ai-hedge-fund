...
| P1-5 | **Web 端筛选一键执行** ✅ | 后端已有 `/hedge-fund/run` | 已实现 `POST /api/screening/auto` + `GET /api/screening/latest`; 前端可通过 Settings 中的「打开选股结果」进入 `ScreeningResultsWithWeights` 工作面 | Web 用户无需 CLI 即可使用核心功能 |
...
| P2-2 | **回测参数对比面板** ✅ | `param_grid.py` 支持参数搜索且前后端已打通 | 已具备 `param-compare-api.ts` + `param-compare-panel.tsx` + 后端 `GET/POST /api/backtest/param-compare`；后续只保留更高级图表增强，不再视为待交付缺口 | 对比不同参数组合的效果 |
...
| # | 功能 | 说明 | 用户价值 |
|---|------|------|----------|
| P2-5 | **自定义策略权重** ✅ | 用户在 Web 端通过滑块调整四个策略的权重(趋势/均值回归/基本面/事件情绪),实时看到推荐变化 | 已实现:`src/screening/custom_weights.py` + `src/main.py --custom-weights` + `POST /api/screening/custom-weights` + 前端 `custom-weights-panel.tsx` / `screening-results-with-weights.tsx` | 高级用户自定义选股偏好 |
