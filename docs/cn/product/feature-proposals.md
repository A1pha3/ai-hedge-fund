...
| R33 | P2 | ✅ | **前门组合预期收益汇总行** | `--top-picks` 底部追加一行：`组合 T+30 预期: +3.2% (加权) | 平均胜率: 58% | BUY 数: 4`。复用 `expected_return.py` 的 T+30 edge 加权平均（样本量 <20 降权 50%），仅当 ≥2 只 BUY 时展示。12 个回归测试（含 partial-winrate 分母 bug 回归：缺失 T+30 胜率的 BUY 不再稀释平均胜率分母）。 |
...
> **最后更新**：2026-06-15（Round 7：R33 平均胜率分母 bug 修复（partial-winrate 不再被稀释）+ R8 死包装 `_render_stop_loss_take_profit` 移除（测试重定向到 `_compute_pick_risk_advice`））
