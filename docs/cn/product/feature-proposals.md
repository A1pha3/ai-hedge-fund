...
| R37 | P1 | ⏳ | **回测价格复权策略明确化** | 当前回测价格路径统一走 Tushare `daily`（不复权），且无任何分红/送股/配股 corporate-action 处理。跨除权除息日的收益计算会被除权（送股/分红）污染，产生假缺口，系统性高估回撤/低估收益。**需要产品决策**：是否切换到前复权（qfq）并接受所有历史回测结果变化，或保持不复权但在前门/回测输出明确披露"价格未复权、跨除权日收益不可直接比较"。候选实现：`get_ashare_prices_with_tushare` 改用 `pro_bar(adj="qfq")`，或在不复权路径加披露说明 + 除权日标记。阻塞原因：切换复权会改变所有历史结果，属用户决策范围。 |
| R38 | P2 | ⏳ | **回测交易日历用 A 股真实交易日（trade_cal）** | `engine_market_data.iter_backtest_dates` 用 pandas `freq="B"`（周一-周五通用工作日），不排除中国节假日（春节/国庆等）。节假日时 `load_current_prices` 回退到前一个交易日收盘价，产生 phantom zero-return bar，轻微稀释 Sharpe/年化（多几个零收益日）。项目已有 `get_open_trade_dates`（trade_cal）但未被 engine 使用。候选实现：`iter_backtest_dates` 优先用 `get_open_trade_dates`，失败 fallback 到 `freq="B"`。阻塞原因：引入网络依赖到日期迭代，且会改变所有回测的 bar 数，属 backtest 核心路径变更。 |
| R39 | P1 | ⏳ | **composite_score fallback domain mismatch（score_b → composite_score）** | 当 ticker 不在 composite_report 中（composite 计算异常或超出 top_n），`rank_recommendations_by_investability` 的 fallback 把 `score_b`（域 [0,1]）直接赋给 `composite_score`（域 [-1,1]，含负 penalties）。score_b=0.55 的 missing-composite ticker 绕过 consistency/momentum/sector 负调整，错误地以 0.55 跨越 BUY 门控（0.5），可能把应降级为 HOLD 的标的错误标记为 BUY。**需要设计决策**：移除 fallback（composite_score=None 时强制 AVOID/INSUFFICIENT）、加保守 penalty（如 ×0.9）、还是加 `composite_verified` 标记让 BUY 门控额外检查。阻塞原因：改变 fallback 语义会影响所有 missing-composite ticker 的 ranking/verdict，需明确产品意图。 |
| R40 | P2 | ⏳ | **macro_data look-ahead（fetch_macro_snapshot 无 trade_date 过滤）** | `src/data/macro_data.py` 的 `fetch_macro_snapshot()` 拉取"最新"宏观数据发布，无 `as_of`/`trade_date` 过滤。回测/replay 场景下会读到模拟交易日之后发布的宏观数据（point-in-time look-ahead）。当前仅作为 `state.macro_context` 的 informational label，**未被** `_resolve_regime_gate` / `classify_btst_regime_gate` / 仓位缩放 / risk-off 决策消费，故不 corrupt 任何交易决策，属 latent。候选实现：`fetch_macro_snapshot(as_of=trade_date)` 过滤 `announce_date <= trade_date`。阻塞原因：当前无决策消费方，优先级低于 R37/R38/R39。 |
