...
| R45 | P3 | ❌ | **连续推荐 streak 跨长假误算修复（R36 同族：交易日历近似残留）** | Campaign 21 research refill 发现：`_prev_trading_day`（`consecutive_recommendation.py:132`）用 weekday 近似（跳周六/周日）计算 streak 步进，与 R38 同族——R38 已修 backtest 日期迭代用真实 trade_cal，但 streak 计算仍用 weekday 近似。**影响**：春节/国庆等 7-9 天长假期间，weekday 近似把假期内的周五误当成"上一个交易日"，导致节前最后交易日与节后首个交易日的推荐被误判为"连续"，虚增 R4 streak bonus（`compute_consecutive_recommendations`）。量化：2024 春节 2024-02-08（Thu 节前最后日）→ 2024-02-19（Mon 节后首日），真实间隔 7 个交易日，但 weekday 近似返回 2024-02-16（Fri 假期内休市日）→ 误判 consecutive。每年约 2-3 次长假（春节+国庆+五一），影响这些窗口的 streak 加权。next_smallest_slice: 复用 R38 的 `get_open_trade_dates` 或提取共享 `_prev_real_trading_day` helper，fallback 到 weekday 近似（无 token/网络失败时保持当前行为）。deferred：低频影响（年 2-3 次）+ 需权衡 trade_cal API 依赖。 |
...
> **最后更新**：2026-06-17（Campaign 21：完成 R43 PIT 不变式集成测试；research refill R45 连续推荐 streak 跨长假误算）
