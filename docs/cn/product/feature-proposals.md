...
| # | 功能 | 优先级 | 状态 |
|---|------|--------|------|
| P14-1 | **多时间框架趋势共振** — 对每只推荐标的计算 5d/20d/60d 价格趋势方向 (线性回归斜率), 三个方向一致时给予共振 bonus (+0.05), 方向冲突时惩罚 (-0.05); 融入 composite_score 作为第 6 个信号因子 | P0 | 🔄 |
| P14-2 | **Top picks 相关性过滤** — `--top-picks` 输出时检查推荐标的之间的行业重叠, 若 3+ 只来自同一申万行业则发出集中度警告并建议替换 | P1 | ❌ |
...
| Phase 17 (信号精度最终打磨) | **0/2 (0%)** 🔄 | P14-1 🔄 / P14-2 ❌ |
| **后端** | **100%** 🎉 | — |
| **CLI** | **100%** 🎉 | — |
| **前端** | **100%** 🎉 | — |
