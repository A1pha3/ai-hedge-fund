...
> **⚠ 状态标注 (2026-07-10)**: 本文为 **MR 因子翻转前**的调研快照。owner 已于 **2026-06-30 commit `023acd74`** 基于 NS-4 倒挂信号翻转 4 个 mean_reversion 子因子方向 (RSI 极值/布林带 z-score/统计套利/Hurst regime，超卖→看空、超买→看多，适配 T+1 短期动量主导市场)，倒挂的结构性根因已处理。本文下述的倒挂数字 (low 50.5% → high 39.5%) 为翻转前老模型测量，翻转后效果待新模型 T+5/T+10 数据累积后由 NS-7 重评。本文保留作调研方法论参考，**不代表当前模型状态**。
