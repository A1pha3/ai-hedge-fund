...
- 任何进入 `quarantine` 或 `separate_surface` 的 ticker，都应保持治理隔离；本轮代码级 enforcement 只覆盖 round1 因子研究默认跳过，是否进一步阻断 `docs/prompt/find_actor/` 或 `ai-hedge-fund-btst` 仍是后续治理 / 集成决策。
...
- 任何处于 `quarantine` 或 `separate_surface` 的 ticker，在治理解除前都必须保持 fail-closed；但本轮已落地的代码级 enforcement 仅为 round1 research-surface 过滤，是否扩展到 `docs/prompt/find_actor/` 或 `ai-hedge-fund-btst` 仍待后续周期单独接线和决策。
