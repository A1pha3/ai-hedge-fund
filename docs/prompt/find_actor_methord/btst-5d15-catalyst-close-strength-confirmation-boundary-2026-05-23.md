...
在 `close_strength <= 0.89` 之上，又试了两类更紧的约束：
...
也就是说，在当前这批 catalyst 窄 gate 样本里：
...
本质上是在切出同一个更窄子集，不该把它们误判成两条可以叠加的独立 alpha。

这条更窄组合的收益质量确实更高，但样本只剩 4 个 closed，远比 `close_strength <= 0.89` 本身更容易过拟合。所以当前应该把它看成：
...
3. 在 `close_strength <= 0.89` 的样本还没明显扩张前，不要急着把 `breakout_freshness <= 0.40` 或 `volume_expansion_quality <= 0.25` 写成正式组合规则；先把它们当作同一批超窄子集观察。
4. 如果 `close_strength <= 0.89` 在样本变多后仍能保持 55% 以上命中率，再看这个超窄子集是否同步扩张并继续维持高赔率。
5. 如果样本扩大到接近 30 时，这条边界开始退化，再回头检查是不是 `0.892` 或 breakout/volume 的组合更稳。
