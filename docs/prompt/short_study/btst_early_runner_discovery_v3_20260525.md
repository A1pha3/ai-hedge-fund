...
v2 已经解决了方向问题：不要追已经涨太多的强票，要在 full report 前面加早期预警层。v3 把这个方向继续往落地推进，重点补 6 个护栏：

1. 把收盘后的 `early_runner_pre_score` 和次日盘中的 `early_runner_confirm_score` 分开，避免前视偏差。
2. 加入 `market_regime_gate`，让市场状态决定是否允许交易、减仓或只做观察。
3. 在回测里显式模拟滑点、手续费、涨停买不到、一字板买不到和高开放弃。
4. 把 10 日 +50% 降为右尾分析标签，不作为主优化目标。
5. 给已经大涨的强票新增 `second_entry_reentry` 二买模块，避免和早期首买逻辑混在一起。
6. 建立失败样本日志，让系统同时学习“为什么涨起来”和“为什么看起来强却失败”。
...
早期主升浪策略不是每天都该用。市场整体退潮时，动量信号会突然失效，强票也容易从“主升浪”变成“冲高回落”。所以在进入个股预警前，要先算 `market_regime_gate`。

建议字段：

```text
index_trend_score        = major_index_close_above_ma5_ma10_ma20
market_breadth_score     = advancing_stock_ratio and limit_up_count
turnover_heat_score      = total_market_turnover_rank_60d
theme_diffusion_score    = hot_theme_count and theme_breadth_median
crash_risk_score         = big_down_count + volatility_spike + index_gap_down
```
...
| 市场状态 | 条件示意 | 动作 |
| --- | --- | --- |
| GREEN | 指数结构不弱，涨多跌少，主题扩散 | A / B 级预警正常输出 |
| YELLOW | 指数震荡，主题分化，成交额一般 | 只允许 A 级，仓位减半 |
| RED | 指数破位、跌停或大跌样本增多、波动率急升 | 只记录观察，不升级交易候选 |

这层不应该太复杂。先用仓库已有 `src/screening/market_state.py` 和 `src/screening/market_state_helpers.py` 能拿到的市场状态做第一版，宁可粗一点，也要先避免在系统性退潮日追右侧。
...
regime_penalty =
  0.10 if market_regime == YELLOW
+ 0.25 if market_regime == RED
...
| T 日收盘后派生 | `theme_breadth_score`、`historical_prior_score`、`market_regime` | 是 | 是 |
...
- 市场状态字段优先复用 src/screening/market_state.py 和 src/screening/market_state_helpers.py 能生成的指标；
...
| early_runner + 二买模块 | 早期首买和高位二买分开建模 | 不浪费强票后续机会，规则更干净 | 实现复杂度更高 | v3 推荐路线 |
...
第三步，加入 `market_regime_gate`，先做到 RED 状态只观察、YELLOW 状态减仓。
...
v3 最值得优先实现的是“双分数 + 市场门控 + 成交模拟”的窄门：
...
这条路不会每次都抓到 10 日 +50%，但它比追 5 日已涨 30% 的股票更接近你要的提前布局，也更容易把亏损控制在可承受范围内。已经大涨的强票不要硬塞进首买逻辑，交给 `second_entry_reentry` 等回踩换手后的二买机会。
